const WEIGHTS = {
  ...
  timeRequired: 0.04,
  // shorter = slightly higher
  reversibility: 0.04,
  // irreversible = slightly higher
  dependencyImpact: 0.04,
  buddyCanHandle: 0.02,
  // if Buddy can handle, marginally lower user-priority
  contextRelevance: 0.02
};
function scoreRecommendation(r, prefs) {
  const peopleNorm = Math.min(1, r.peopleAffected / 5);
  const financeNorm = Math.min(1, r.financialImpact / 5e4);
  const timeNorm = 1 - Math.min(1, r.estimatedMinutes / 60);
  const revNorm = r.reversible ? 0 : 1;
  let s = WEIGHTS.urgency * r.urgency + WEIGHTS.consequenceOfDelay * r.impact + WEIGHTS.peopleAffected * peopleNorm + WEIGHTS.financialImpact * financeNorm + WEIGHTS.personalImportance * r.emotionalImportance + WEIGHTS.emotionalImportance * r.emotionalImportance + WEIGHTS.timeRequired * timeNorm + WEIGHTS.reversibility * revNorm + WEIGHTS.dependencyImpact * peopleNorm + WEIGHTS.buddyCanHandle * (r.buddyCanHandle ? -1 : 1) * 0.5 + WEIGHTS.contextRelevance * 0.5;
...
function rankPriorities(input) {
  const items = input.recommendations.filter((r) => r.status === "suggested" || r.status === "accepted").map((r) => {
    ...
    return {
      ...
      tier: tierFor(score),
      score,
      explanation: reasonFor(r),
      buddyCanHandle: r.buddyCanHandle,
      estimatedMinutes: r.estimatedMinutes
    };
  });
  for (const c of input.conflicts.filter((c2) => c2.status === "open")) {
  ...
  items.sort((a, b) => {
    ...
    if (am !== void 0 && bm !== void 0) return am - bm;
    if (am !== void 0) return -1;
    if (bm !== void 0) return 1;
