...
function buildRecommendations(ctx) {
  ...
  recs.push({
    ...
    reversible: true,
    buddyCanHandle: true,
    requiredPermission: "calendar.block_focus",
  ...
  recs.push({
    ...
    reversible: true,
    buddyCanHandle: true,
    requiredPermission: "calendar.move_internal",
  ...
  recs.push({
    ...
    impact: 0.7,
    peopleAffected: 0,
    financialImpact: SIGNALS.finance.amount,
...
function buildSummary(ctx, recs, conflictsOpen, planState) {
  ...
  const focusClause = focus > 0 ? `protect ${ctx.signals.focusPreference.start}–${ctx.signals.focusPreference.end} for focused work` : `there's no clear focus window — Buddy will try to carve one out`;
  const familyClause = ctx.signals.family.title ? ` Leave by ${ctx.signals.family.time} for ${ctx.signals.family.title}.` : "";
  const narrative = meetings > 0 ? `You have ${meetings} meeting${meetings === 1 ? "" : "s"} today, but ${activePart}. Tackle what's blocking others first, and ${focusClause}.${familyClause}` : `A light calendar today — a good chance to ${focusClause} and clear what's due.${familyClause}`;
  return {
    meetings,
    activeMeetings: active,
    criticalTasks: critical,
    conflicts: conflictsOpen,
    focusMins: focus,
    personalCommitments: 2,
    estTimeSavedMins: saved,
    updatedAt: (/* @__PURE__ */ new Date()).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }),
    narrative
  };
}
...
const COMPARISON_CAVEAT = "Estimates, not guarantees — completion odds are rough reads from similar past days.";
export {
  CATEGORY_LABELS,
  COMPARISON_CAVEAT,
