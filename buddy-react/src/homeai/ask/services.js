...
const GRANTED = /* @__PURE__ */ new Set([
  "Send email as Antara",
  "Send email to external recipients",
  "Create issues in Project Phoenix",
  "Assign issues & notify",
  "Edit issues",
...
function runnerFor(step) {
  return async (s) => {
    switch (s.id) {
      case "st-internal":
        return okE(await AskServices.email.send({ to: new Array(6).fill("x"), subject: "Project Phoenix — Weekly Review MOM (internal)", external: false }));
      case "st-external":
        return okE(await AskServices.email.send({ to: new Array(2).fill("x"), subject: "Project Phoenix — Meeting notes", external: true }));
      case "st-create":
        return okE(await AskServices.jira.createIssue({ title: "4 confirmed action items", description: "", sprint: "Sprint 24", due: "various" }));
      case "st-assign": {
      ...
      case "st-refs":
        return okE({ kind: "jira", id: `REF-${++seq}`, label: "Deadlines & transcript refs added", actor: "Buddy (simulated)", at: Date.now(), detail: "4 tasks linked to transcript moments" });
      case "st-timeline":
        return okE(await AskServices.timeline.update("26 Aug", "28 Aug"));
      case "st-schedule":
        return okE(await AskServices.calendar.create({ title: "Stakeholder review", date: "25 Jul 2026", attendees: 6 }));
      case "st-remind":
        return okE(await AskServices.monitor.activate("Reminders 24h before each deadline"));
      case "st-overdue":
        return okE(await AskServices.monitor.activate("Overdue escalation to Antara"));
      default:
        return okE({ kind: "monitor", id: evId("X"), label: "Done", actor: "Buddy (simulated)", at: Date.now(), detail: s.title });
    }
  };
}
function okE(evidence) {
