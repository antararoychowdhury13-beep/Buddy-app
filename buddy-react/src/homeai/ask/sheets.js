...
import { GRANTED, runnerFor } from "./services";
const CONF_META = {
...
function closeTop() {
  ...
  scrims[scrims.length - 1]?.querySelector(".sheet-close")?.click();
}
function closeAll() {
  document.querySelectorAll(".sheet-scrim .sheet-close").forEach((b) => b.click());
}
function openConsent(store, rerender, onMomReady) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      ...
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
        closeTop();
        toast("Capture declined — nothing recorded.");
        store.meetingTo("upcoming");
        rerender();
      } }, ["Decline"])
    ]),
...
function openCompanion(store, rerender, onMomReady, path) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "companion-ctrls" }, [
      ...
      ctrl(offRecord ? "Back on record" : "Off record", () => {
        offRecord = !offRecord;
        stateLabel.textContent = offRecord ? "Off the record" : path === "record" ? "Recording" : "Notes only";
        toast(offRecord ? "Off the record — not captured." : "Back on record.");
      })
    ]),
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
      ...
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        ...
        store.meetingTo("processing");
        closeTop();
        openProcessing(store, rerender, onMomReady);
      } }, ["End meeting"])
    ]),
...
function openProcessing(store, rerender, onMomReady) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    list,
    el("button", { class: "btn btn-ghost full", type: "button", style: "margin-top:14px;", onClick: () => {
      closeTop();
      toast("Processing continues in the background.");
...
function openMomWorkspace(store, rerender) {
  const mom = buildMom();
  ...
  function render() {
    const versions = [
      ["internal", "Internal", "Full detail"],
      ["external", "Client-safe", "Confidential removed"],
      ["leadership", "Leadership", "Summary + decisions"],
      ["actions_only", "Actions only", "Tasks + owners"]
    ];
    const decisions = store.data.launchCorrected ? mom.decisions.map((d) => d.id === "d1" ? { ...d, text: "Beta launch moved to 28 August." } : d) : mom.decisions;
    ...
    const nodes = [
      // version switcher
      el("div", { class: "version-switch", role: "radiogroup", "aria-label": "MOM version" }, versions.map(([a, label, note]) => el("button", { class: `ver-btn${audience === a ? " active" : ""}`, role: "radio", "aria-checked": audience === a, type: "button", onClick: () => {
        audience = a;
        store.data.activeAudience = a;
        store.save();
        render();
      } }, [
        el("div", { class: "ver-name" }, [label]),
        el("div", { class: "ver-note" }, [note])
      ]))),
      audience === "external" ? el("div", { class: "redact-banner", role: "status" }, [
        ...
        store.data.externalRedacted ? null : el("button", { class: "btn btn-primary sm", type: "button", onClick: () => {
          store.data.externalRedacted = true;
          store.audit("Redacted external MOM", "Removed budget, staffing, vendor notes, recording link");
          store.save();
          render();
          toast("Confidential content removed from the client-safe version.");
        } }, ["Remove confidential"])
      ]) : null,
      // meeting details
      ...
      ...sections.map((s) => {
        ...
        if (redacted) return el("div", { class: "mom-section redacted" }, [el("div", { class: "mom-sec-head" }, [el("h3", {}, [s.title]), el("span", { class: "redact-tag" }, ["Removed for this audience"])])]);
        const bodyText = store.data.momEdits[s.id] ?? s.body;
        return el("div", { class: `mom-section${s.confidential ? " confidential" : ""}` }, [
          el("div", { class: "mom-sec-head" }, [
            el("h3", {}, [s.title]),
            s.confidential ? el("span", { class: "conf-tag" }, [el("span", { class: "ic", html: ICONS.shield }), "Confidential"]) : null,
          ...
          el("div", { style: "flex:1;" }, [
            ...
            d.corrected && !store.data.launchCorrected ? el("div", { class: "conflict-note" }, [
              ...
              el("button", { class: "chip-btn primary", type: "button", onClick: () => {
                store.data.launchCorrected = true;
                store.audit("Corrected decision", "Launch date confirmed 28 August");
                store.save();
                render();
                toast("Confirmed — 28 August.");
              } }, ["Confirm 28 Aug"])
            ]) : null
          ])
        ]))
        ...
        el("table", { class: "action-table" }, [
          ...
          el("tbody", {}, mom.actionItems.filter((a) => !store.data.momRemoved.includes(a.id)).map((a) => el("tr", {}, [
            ...
            el("td", {}, [
              ...
              a.confidence !== "high" && !store.data.momConfirmed.includes(a.id) ? el("button", { class: "chip-btn primary", type: "button", style: "margin-top:6px;", onClick: () => {
                store.data.momConfirmed.push(a.id);
                store.save();
                render();
                toast("Confirmed.");
              } }, ["Confirm"]) : null,
              el("button", { class: "chip-btn ghost", type: "button", style: "margin-top:6px;", onClick: () => {
                store.data.momRemoved.push(a.id);
                store.save();
                render();
                toast("Removed — you can restore it.");
              } }, ["Remove"])
            ])
          ])))
        ]),
        store.data.momRemoved.length ? el("button", { class: "btn btn-ghost sm", type: "button", style: "margin-top:8px;", onClick: () => {
          store.data.momRemoved = [];
          store.save();
          render();
          toast("Restored removed items.");
        } }, [el("span", { class: "ic", html: ICONS.undo }), "Restore removed"]) : null
      ]) : null,
      // open questions
      el("div", { class: "mom-section" }, [
  ...
  function editSection(id, title, current) {
    ...
    ta.value = current;
    openSheet({ title: `Edit · ${title}`, body: el("div", { class: "sheet-body" }, [
      ta,
      el("button", { class: "btn btn-primary full", type: "button", style: "margin-top:12px;", onClick: () => {
        store.data.momEdits[id] = ta.value;
        store.save();
        closeTop();
        render();
...
function openPlanFlow(store, rerender) {
  ...
  function render() {
    ...
    wrap.replaceChildren(
      el("p", { class: "muted", style: "margin-bottom:12px;" }, ["Review, edit, enable/disable and approve. High-impact steps need explicit sign-off before anything runs."]),
      // assumptions
    ...
    const bar = el("div", { class: "approval-bar" }, [
      ...
      el("button", { class: "btn btn-primary", type: "button", disabled: !allApproved, onClick: () => {
        store.audit("Approved plan", `${enabled.length} steps`);
        closeTop();
        openExecution(store, rerender);
      } }, [allApproved ? "Approve & execute" : `Approve ${needApproval.length - needApproval.filter((s) => store.data.approvedStepIds.includes(s.id)).length} more`])
    ]);
    wrap.append(bar);
  }
  function planStepCard(s, i) {
    ...
    return el("div", { class: `plan-step${s.enabled ? "" : " disabled"}`, "data-step": s.id }, [
      el("div", { class: "ps-top" }, [
        ...
        el("button", { class: `toggle sm${s.enabled ? " on" : ""}`, type: "button", role: "switch", "aria-checked": s.enabled, "aria-label": `${s.enabled ? "Disable" : "Enable"} ${s.title}`, onClick: () => {
          s.enabled = !s.enabled;
          if (!s.enabled) store.data.approvedStepIds = store.data.approvedStepIds.filter((id) => id !== s.id);
          store.save();
          render();
        } }, [s.enabled ? "On" : "Off"])
      ]),
      ...
      s.enabled && s.approvalRequired ? el("label", { class: `ps-approve${approved ? " approved" : ""}` }, [
        el("input", { type: "checkbox", checked: approved, "aria-label": `Approve ${s.title}`, onChange: (e) => {
          const c = e.target;
          if (c.checked) {
            if (!store.data.approvedStepIds.includes(s.id)) store.data.approvedStepIds.push(s.id);
          } else store.data.approvedStepIds = store.data.approvedStepIds.filter((id) => id !== s.id);
          store.save();
          render();
        } }),
  ...
  function previewButton(s) {
    if (s.application === "Outlook" && s.id.includes("MOM") || s.id === "st-internal" || s.id === "st-external") return el("button", { class: "chip-btn", type: "button", onClick: () => openEmailPreview(s.id === "st-external") }, ["Preview email"]);
    if (s.application === "Jira") return el("button", { class: "chip-btn", type: "button", onClick: () => openJiraPreview() }, ["Preview tasks"]);
    if (s.id === "st-timeline") return el("button", { class: "chip-btn", type: "button", onClick: () => openTimelinePreview() }, ["Preview timeline change"]);
    if (s.id === "st-schedule") return el("button", { class: "chip-btn", type: "button", onClick: () => openCalendarPreview() }, ["Preview calendar"]);
    return null;
  }
  function editAssumption(id, current) {
    const inp = el("input", { type: "text", class: "edit-input", "aria-label": "Edit assumption" });
    inp.value = current;
    openSheet({ title: "Edit assumption", body: el("div", { class: "sheet-body" }, [
      inp,
      el("button", { class: "btn btn-primary full", type: "button", style: "margin-top:12px;", onClick: () => {
        const a = store.data.assumptions.find((x) => x.id === id);
...
function kv(k, v) {
  return el("div", { class: "ps-kv" }, [el("span", { class: "meta-k" }, [k]), el("span", { class: "meta-v" }, [v])]);
}
function summaryRow(k, v) {
  return el("div", { class: "sum-row" }, [el("span", { class: "sum-k" }, [k]), el("span", { class: "sum-v" }, [v])]);
}
...
function openExecution(store, rerender) {
  ...
  function statusBadge(s) {
    const map = {
      queued: ["Queued", PALETTE.slate],
      in_progress: ["Running", PALETTE.blue],
      completed: ["Completed", PALETTE.green],
      failed: ["Needs attention", PALETTE.red],
      partially_completed: ["Partial", PALETTE.amber],
      waiting_for_approval: ["Waiting", PALETTE.amber]
    };
  ...
  const stopBar = el("div", { class: "exec-stopbar" }, [
    ...
    el("button", { class: "btn btn-ghost danger sm", type: "button", onClick: () => {
      for (const s of steps) if (s.status === "queued" || s.status === "in_progress") s.status = "cancelled";
      store.save();
      render();
      toast("Execution stopped.");
      store.audit("Emergency stop", "User halted execution");
    } }, ["Stop"])
  ]);
  const body = el("div", { class: "sheet-body" }, [
  ...
  function finish() {
    ...
    setTimeout(() => {
      closeAll();
      store.pushMessage({ kind: "completion" });
      setTimeout(() => {
        store.pushMessage({ kind: "followup" });
  ...
  function recovery(label, onClick) {
    return el("button", { class: "chip-btn", type: "button", onClick }, [label]);
  }
}
function openEvidence(s) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    s.reversible === "fully_reversible" || s.reversible === "partially_reversible" ? el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
      s.status = "undone";
      toast("Action undone.");
      closeTop();
    } }, [el("span", { class: "ic", html: ICONS.undo }), "Undo this action"]) : el("p", { class: "fineprint" }, ["This action can't be reliably undone."])
  ]);
  openSheet({ title: "Evidence", body });
}
function openCompletion(store, rerender) {
  const rows = [
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        closeTop();
        openWorkflowBuilder(store, rerender);
      } }, [el("span", { class: "ic", html: ICONS.spark }), "Turn this into a reusable workflow"]),
      ...
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
        closeTop();
        openMonitoring(store, rerender);
      } }, ["Set up follow-up monitoring"])
    ])
  ]);
  openSheet({ title: "Completion summary", body });
}
function openMonitoring(store, rerender) {
  let handled = false;
  const wrap = el("div", {});
  function render() {
    wrap.replaceChildren(
      el("div", { class: "monitor-alert" }, [
      ...
      handled ? el("div", { class: "resolved-banner" }, [el("span", { class: "ic", html: ICONS.check }), el("span", {}, ["Buddy sent Rahul a standard check-in and logged it. Evidence: MON-check-in."])]) : el("div", { class: "monitor-actions" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
          handled = true;
          store.audit("Monitoring action", "Sent Rahul a standard check-in");
          store.save();
          render();
          toast("Check-in sent to Rahul · evidence recorded.");
        } }, ["Ask Rahul for an update"]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          toast("Deadline extended by 2 days.");
          store.audit("Monitoring action", "Extended deadline");
        } }, ["Extend deadline"]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          toast("Reassign flow (simulated).");
        } }, ["Reassign"]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          toast("PM notified.");
          store.audit("Monitoring action", "Notified project manager");
...
function openWorkflowBuilder(store, rerender) {
  const wf = {
  ...
  function render() {
    wrap.replaceChildren(
      ...
      field("Name", wf.name),
      field("Trigger", wf.trigger),
      field("Scope", "Work → Project Phoenix"),
      field("Connected apps", "Teams, Outlook, Jira"),
      field("Approval points", wf.approval),
      field("Reminders", "24h before each deadline"),
      field("Escalation", "Notify Antara when overdue"),
      field("Data retention", `${wf.retention} days`),
      el("div", { class: "info-block" }, [
        el("div", { class: "info-label" }, ["Autonomy level"]),
        el("div", { class: "autonomy-row" }, [["ask_each_time", "Ask each time"], ["plan_then_act", "Plan, then act on approval"], ["auto_low_risk", "Auto-run low-risk only"]].map(([k, label]) => el("button", { class: `auton-btn${wf.autonomy === k ? " active" : ""}`, type: "button", "aria-pressed": wf.autonomy === k, onClick: () => {
          wf.autonomy = k;
          render();
        } }, [label])))
      ]),
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
          store.data.workflowSaved = true;
          store.transition("workflow_saved");
          store.audit("Activated workflow", wf.name);
          store.save();
          closeTop();
          rerender();
          toast("Workflow activated — it keeps your approval points.");
        } }, ["Activate workflow"]),
        el("button", { class: "btn btn-tonal full", type: "button", onClick: () => {
          toast("Test run — Buddy simulated the workflow end to end.");
        } }, ["Test workflow"]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          store.data.workflowSaved = true;
          store.save();
  ...
  function field(k, v) {
    return el("div", { class: "wf-field" }, [el("span", { class: "wf-k" }, [k]), el("span", { class: "wf-v" }, [v])]);
  }
  render();
...
function openActivity(store) {
  ...
  openSheet({ title: "Activity & audit", body: el("div", { class: "sheet-body" }, [
    ...
    items.length === 0 ? el("p", { class: "empty" }, ["No activity yet — start the meeting assistant to see the trail."]) : el("div", { class: "audit-list" }, items.map((a) => el("div", { class: "audit-row" }, [
    ...
    el("button", { class: "btn btn-ghost sm danger full", type: "button", style: "margin-top:14px;", onClick: () => {
      store.reset();
      closeAll();
      toast("Ask Buddy demo reset.");
    } }, ["Reset Ask Buddy demo"])
  ]) });
}
export {
  openActivity,
