...
import type { MomAudience, PlanStep } from "./models";
import {
  ACTION_ITEMS, ASSUMPTIONS, CONSENT, DECISIONS, MEETING, PALETTE, PARTICIPANTS, USER, buildMom,
} from "./mock";
import { GRANTED, runnerFor } from "./services";
import type { AskStore } from "./store";
...
function confChip(k: string): HTMLElement {
...
function closeTop() { const scrims = document.querySelectorAll<HTMLElement>(".sheet-scrim"); (scrims[scrims.length - 1]?.querySelector(".sheet-close") as HTMLElement)?.click(); }
function closeAll() { document.querySelectorAll<HTMLElement>(".sheet-scrim .sheet-close").forEach((b) => b.click()); }

// ============ CONSENT ============
export function openConsent(store: AskStore, rerender: () => void, onMomReady: () => void) {
  ...
  function choose(path: "record" | "notes_only") {
    store.data.consentPath = path; store.save();
...
export function openCompanion(store: AskStore, rerender: () => void, onMomReady: () => void, path: "record" | "notes_only") {
  store.meetingTo(path === "record" ? "recording" : "ready");
  let seconds = 0; let paused = false; let offRecord = false;
  const timer = el("span", { class: "rec-timer" }, ["00:00"]);
  const stateLabel = el("span", { class: "rec-state" }, [path === "record" ? "Recording" : "Notes only"]);
  const interval = window.setInterval(() => { if (!paused) { seconds++; timer.textContent = `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`; } }, 1000);
  ...
  const ctrl = (label: string, onClick: () => void, cls = "chip-btn") => el("button", { class: cls, type: "button", onClick }, [label]);
...
// ============ PROCESSING ============
export function openProcessing(store: AskStore, rerender: () => void, onMomReady: () => void) {
...
// ============ MOM WORKSPACE ============
export function openMomWorkspace(store: AskStore, rerender: () => void) {
  const mom = buildMom();
  let audience: MomAudience = store.data.activeAudience;

  const wrap = el("div", {});
  function isRedactedForAudience(sectionConfidential: boolean): boolean {
    return sectionConfidential && (audience === "external" || audience === "actions_only");
  }

  function render() {
    const versions: [MomAudience, string, string][] = [
    ...
    const nodes: (HTMLElement | null)[] = [
      // version switcher
      el("div", { class: "version-switch", role: "radiogroup", "aria-label": "MOM version" }, versions.map(([a, label, note]) =>
        el("button", { class: `ver-btn${audience === a ? " active" : ""}`, role: "radio", "aria-checked": audience === a, type: "button", onClick: () => { audience = a; store.data.activeAudience = a; store.save(); render(); } }, [
    ...
    wrap.replaceChildren(...(nodes.filter(Boolean) as HTMLElement[]));
  }
  function editSection(id: string, title: string, current: string) {
    const ta = el("textarea", { class: "edit-ta", rows: "4", "aria-label": `Edit ${title}` }) as HTMLTextAreaElement;
    ta.value = current;
...
function openTranscript(ref: string, title: string) {
...
function infoBlock(label: string, node: HTMLElement): HTMLElement {
  return el("div", { class: "info-block" }, [el("div", { class: "info-label" }, [label]), node]);
}

// ============ PLAN + PREVIEWS + APPROVAL ============
export function openPlanFlow(store: AskStore, rerender: () => void) {
  ...
  function planStepCard(s: PlanStep, i: number): HTMLElement {
    ...
    return el("div", { class: `plan-step${s.enabled ? "" : " disabled"}`, "data-step": s.id }, [
      el("div", { class: "ps-top" }, [
        ...
        el("button", { class: `toggle sm${s.enabled ? " on" : ""}`, type: "button", role: "switch", "aria-checked": s.enabled, "aria-label": `${s.enabled ? "Disable" : "Enable"} ${s.title}`, onClick: () => { s.enabled = !s.enabled; if (!s.enabled) store.data.approvedStepIds = store.data.approvedStepIds.filter((id) => id !== s.id); store.save(); render(); } }, [s.enabled ? "On" : "Off"]),
      ]),
      ...
      s.enabled && s.approvalRequired ? el("label", { class: `ps-approve${approved ? " approved" : ""}` }, [
        el("input", { type: "checkbox", checked: approved, "aria-label": `Approve ${s.title}`, onChange: (e: Event) => { const c = e.target as HTMLInputElement; if (c.checked) { if (!store.data.approvedStepIds.includes(s.id)) store.data.approvedStepIds.push(s.id); } else store.data.approvedStepIds = store.data.approvedStepIds.filter((id) => id !== s.id); store.save(); render(); } }),
        el("span", {}, [`I approve this ${s.risk === "high" ? "high-impact" : ""} step (${s.riskLabel.toLowerCase()})`]),
      ]) : null,
    ]);
  }

  function previewButton(s: PlanStep): HTMLElement | null {
  ...
  function editAssumption(id: string, current: string) {
    const inp = el("input", { type: "text", class: "edit-input", "aria-label": "Edit assumption" }) as HTMLInputElement;
    inp.value = current;
    openSheet({ title: "Edit assumption", body: el("div", { class: "sheet-body" }, [inp,
      el("button", { class: "btn btn-primary full", type: "button", style: "margin-top:12px;", onClick: () => { const a = store.data.assumptions.find((x) => x.id === id); if (a) a.text = inp.value; store.save(); closeTop(); render(); toast("Assumption updated."); } }, ["Save"]),
    ]) });
...
function kv(k: string, v: string): HTMLElement { return el("div", { class: "ps-kv" }, [el("span", { class: "meta-k" }, [k]), el("span", { class: "meta-v" }, [v])]); }
function summaryRow(k: string, v: string): HTMLElement { return el("div", { class: "sum-row" }, [el("span", { class: "sum-k" }, [k]), el("span", { class: "sum-v" }, [v])]); }

// ---- action previews ----
function openEmailPreview(external: boolean) {
  const body = el("div", { class: "sheet-body" }, [
...
function previewField(k: string, v: string): HTMLElement { return el("div", { class: "preview-field" }, [el("span", { class: "pf-k" }, [k]), el("span", { class: "pf-v" }, [v])]); }

// ============ EXECUTION + RECOVERY ============
export function openExecution(store: AskStore, rerender: () => void) {
  store.transition("executing");
  const steps = store.enabledSteps();
  const list = el("div", { class: "exec-timeline" });
  let i = 0;

  function statusBadge(s: PlanStep): HTMLElement {
  ...
  const stopBar = el("div", { class: "exec-stopbar" }, [
    ...
    el("button", { class: "btn btn-ghost danger sm", type: "button", onClick: () => { for (const s of steps) if (s.status === "queued" || s.status === "in_progress") s.status = "cancelled"; store.save(); render(); toast("Execution stopped."); store.audit("Emergency stop", "User halted execution"); } }, ["Stop"]),
  ]);
  ...
  const runNext = async () => {
    ...
    const s = steps[i];
    if (s.status === "cancelled") { i++; return runNext(); }
    s.status = "in_progress"; render(); announce(`Running: ${s.title}`);
    const res = await runnerFor(s)(s);
    if (res.status === "completed") { s.status = "completed"; s.evidence = res.evidence; store.audit("Executed step", s.title); }
    else { s.status = "failed"; s.error = res.error; store.audit("Step needs attention", s.error ?? s.title); }
    store.save(); render();
    i++;
    if (s.status === "failed") return; // pause on the Priya failure until resolved
    setTimeout(runNext, 420);
  };
  setTimeout(runNext, 400);

  function resolvePriya(msg: string, retryAgain = false) {
    store.data.priyaResolution = msg; store.audit("Resolved assignment failure", msg);
    const failStep = steps.find((s) => s.status === "failed");
    if (failStep && !retryAgain) { failStep.status = "partially_completed"; failStep.evidence = { kind: "jira", id: `ASN-RES`, label: "3 of 4 assigned · 1 resolved", actor: "Antara", at: Date.now(), recipients: 3, detail: msg }; }
    store.save(); render(); toast(msg);
    if (!retryAgain) { i++; setTimeout(runNext, 400); }
  }
  function finish() {
    const anyFail = steps.some((s) => s.status === "failed");
    store.transition(anyFail ? "partially_completed" : "completed");
    store.data.monitoringOn = true; store.save();
    stopBar.remove();
  ...
  function recovery(label: string, onClick: () => void): HTMLElement { return el("button", { class: "chip-btn", type: "button", onClick }, [label]); }
}

function openEvidence(s: PlanStep) {
  const e = s.evidence!;
  const body = el("div", { class: "sheet-body" }, [
...
// ============ COMPLETION ============
export function openCompletion(store: AskStore, rerender: () => void) {
...
// ============ MONITORING ============
export function openMonitoring(store: AskStore, rerender: () => void) {
  let handled = false;
  const wrap = el("div", {});
  function render() {
    wrap.replaceChildren(
...
// ============ WORKFLOW BUILDER ============
export function openWorkflowBuilder(store: AskStore, rerender: () => void) {
  const wf = {
    name: "Weekly Project Meeting Assistant",
    trigger: "After a Project Phoenix meeting ends",
    approval: "Ask before external sharing",
    autonomy: "plan_then_act" as "ask_each_time" | "plan_then_act" | "auto_low_risk",
    retention: 30,
  };
  const wrap = el("div", {});
  function render() {
    wrap.replaceChildren(
      ...
      el("div", { class: "info-block" }, [
        ...
        el("div", { class: "autonomy-row" }, ([["ask_each_time", "Ask each time"], ["plan_then_act", "Plan, then act on approval"], ["auto_low_risk", "Auto-run low-risk only"]] as const).map(([k, label]) =>
          el("button", { class: `auton-btn${wf.autonomy === k ? " active" : ""}`, type: "button", "aria-pressed": wf.autonomy === k, onClick: () => { wf.autonomy = k; render(); } }, [label]))),
      ]),
      el("div", { class: "sheet-actions col" }, [
  ...
  function field(k: string, v: string): HTMLElement { return el("div", { class: "wf-field" }, [el("span", { class: "wf-k" }, [k]), el("span", { class: "wf-v" }, [v])]); }
  render();
  openSheet({ title: "Save as workflow", body: el("div", { class: "sheet-body" }, [wrap]) });
}

// ============ ACTIVITY / AUDIT ============
export function openActivity(store: AskStore) {
