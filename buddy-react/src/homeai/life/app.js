...
class LifeApp {
  constructor(root) {
    __publicField(this, "store", new LifeStore());
    __publicField(this, "root");
    this.root = root;
    this.render();
  }
  household() {
    return this.store.data.household === "hh-solo" ? HOUSEHOLD_SOLO : HOUSEHOLD;
  }
  privacy() {
    return this.store.data.privacyMode;
  }
  render() {
    clear(this.root);
    const sections = [
  ...
  privacyStatus() {
    ...
    return el("section", { class: "privacy-bar" }, [
      ...
      el("button", { class: `toggle sm${p ? " on" : ""}`, type: "button", role: "switch", "aria-checked": p, "aria-label": "Toggle privacy mode", onClick: () => {
        this.store.data.privacyMode = !p;
        this.store.data.hideSensitiveValues = !p;
        this.store.audit("Toggled privacy mode", !p ? "on" : "off", "family");
        this.store.save();
        this.render();
        toast(!p ? "Privacy mode on — sensitive values hidden." : "Privacy mode off.");
      } }, [p ? "On" : "Off"])
    ]);
  }
  needsAttention() {
      ...
      ...items.length ? items.map((n) => el("article", { class: "attn-card", style: `border-left:4px solid ${n.accent};` }, [
        ...
        el("div", { class: "attn-actions" }, [
          ...
          el("button", { class: "chip-btn ghost", type: "button", onClick: () => {
            this.store.data.dismissedNudges.push(n.id);
            this.store.audit("Dismissed nudge", n.title, n.category);
            this.store.save();
            this.render();
            toast("Not now.");
          } }, ["Not now"])
  ...
  familyCare() {
    ...
    return el("section", { class: "life-sec", "aria-label": "Family and care" }, [
      ...
      el("article", { class: "care-card", onClick: () => openCare(this.store, () => this.render()) }, [
        ...
        el("div", { class: "care-mini" }, [["Reports", ICONS.doc], ["Meds", ICONS.heart], ["Cab", ICONS.pin], ["Remind", ICONS.clock]].map(([l, i]) => el("span", { class: "care-mini-item" }, [el("span", { class: "ic", html: i }), l]))),
        el("button", { class: "btn btn-primary full", type: "button", style: `background:${PAL.green}; box-shadow:none;`, onClick: (e) => {
          e.stopPropagation();
          openCare(this.store, () => this.render());
        } }, ["Open care journey"])
      ])
    ]);
  }
  ...
  sharedWorkflows() {
    return el("section", { class: "life-sec", "aria-label": "Shared workflows" }, [
      ...
      el("button", { class: "sw-card", type: "button", onClick: () => openSharedWorkflow(this.store, () => this.render()) }, [
        ...
        el("div", { class: "sw-avatars" }, [["AR", PAL.blue], ["RV", PAL.teal], ["CG", PAL.purple]].map(([i, c]) => el("span", { class: "avatar sm", style: `background:${c};` }, [i])))
      ])
    ]);
  }
  footer() {
    return el("section", { class: "foot-controls" }, [
      ...
      el("button", { class: "btn btn-ghost sm", type: "button", onClick: () => {
        this.store.data.household = this.store.data.household === "hh-solo" ? "hh-full" : "hh-solo";
        this.store.save();
        this.render();
        toast(this.store.data.household === "hh-solo" ? "Switched to single-person household." : "Switched to full household.");
      } }, ["Switch household"]),
      el("button", { class: "btn btn-ghost sm danger", type: "button", onClick: () => {
        this.store.reset();
        this.render();
        toast("Life demo reset.");
      } }, ["Reset Life"])
    ]);
  }
}
