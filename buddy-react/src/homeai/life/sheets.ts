/** Life module flow sheets: member detail + info states, pending-inference
 * confirmation, care journey + escalation, wellbeing baseline/correlation/
 * micro-intervention/consent, household basket + store compare, asset/car,
 * occasion + preference-source privacy, money (safe-to-spend/forecast/
 * scenario), high-risk financial action + step-up + receipt, sharing,
 * permissions, activity. */
import { el, openSheet, toast, announce } from "../ui/dom";
import { ICONS } from "../ui/icons";
import type { CareStep, InfoItem, LifeNudge, Member } from "./models";
import {
  BASKET, CAR, CARE_STEPS, CORRELATIONS, FINANCIAL_ACTION, FORECAST_7D, INFO_LEGEND, MICRO,
  OCCASION, PAL, PERMISSION_CATEGORIES, PERMISSION_LABELS, SAFE_TO_SPEND, SCENARIO,
  SHARED_WORKFLOWS, STORES, WELLBEING,
} from "./mock";
import type { LifeStore } from "./store";

function closeTop() { const s = document.querySelectorAll<HTMLElement>(".sheet-scrim"); (s[s.length - 1]?.querySelector(".sheet-close") as HTMLElement)?.click(); }
function stateMeta(state: string) { return INFO_LEGEND.find((l) => l.state === state) ?? INFO_LEGEND[0]; }
function infoChip(state: string): HTMLElement { const m = stateMeta(state); return el("span", { class: "state-chip", style: `color:${m.color}; background:${m.color}1a;` }, [el("span", { class: "dot", style: `background:${m.color};` }), m.label]); }
function visLabel(v: string): string { return { only_you: "Only you", you_and_caregiver: "You & caregiver", household: "Household", custom: "Custom" }[v] ?? v; }
function confMeta(c: string) { return { high: ["High confidence", PAL.green], medium: ["Medium confidence", PAL.amber], low: ["Low confidence", PAL.red] }[c] ?? ["", PAL.slate]; }
function confBadge(c: string): HTMLElement { const [label, color] = confMeta(c); return el("span", { class: "conf-chip", style: `color:${color}; background:${color}1a;` }, [label]); }
function block(label: string, node: HTMLElement): HTMLElement { return el("div", { class: "info-block" }, [el("div", { class: "info-label" }, [label]), node]); }
function kv(k: string, v: string): HTMLElement { return el("div", { class: "preview-field" }, [el("span", { class: "pf-k" }, [k]), el("span", { class: "pf-v" }, [v])]); }

// ============ MEMBER DETAIL + INFO STATES ============
export function openMember(m: Member, store: LifeStore, rerender: () => void) {
  const wrap = el("div", {});
  function render() {
...
function infoRow(i: InfoItem, store: LifeStore, render: () => void, rerender: () => void): HTMLElement {
  ...
  return el("div", { class: `info-item state-${i.state}` }, [
    ...
    needsConfirm ? el("div", { class: "info-actions" }, [
      el("button", { class: "chip-btn primary", type: "button", onClick: () => { store.confirmInfo(i.id); store.audit("Confirmed info", i.label, "family"); render(); rerender(); toast("Confirmed."); } }, ["Confirm"]),
      el("button", { class: "chip-btn", type: "button", onClick: () => { store.correctInfo(i.id); store.audit("Marked not relevant", i.label, "family"); render(); rerender(); toast("Buddy won't use this."); } }, ["Not relevant"]),
      ...
      el("button", { class: "chip-btn ghost danger", type: "button", onClick: () => { store.deleteInfo(i.id); store.audit("Deleted info", i.label, "family"); render(); rerender(); toast("Deleted from Buddy's memory."); } }, ["Delete"]),
    ]),
  ]);
}

function openSourceInspect(i: InfoItem) {
...
function openVisibility(i: InfoItem, store: LifeStore, render: () => void) {
  const opts: [string, string][] = [["only_you", "Only you"], ["you_and_caregiver", "You & caregiver"], ["household", "Household"]];
  openSheet({ title: "Who can see this", body: el("div", { class: "sheet-body" }, [
    el("p", { class: "muted", style: "margin-bottom:12px;" }, [`"${i.label}"`]),
    ...opts.map(([v, label]) => el("button", { class: `btn btn-ghost full${i.visibility === v ? " ctx-active" : ""}`, type: "button", onClick: () => { i.visibility = v as any; store.audit("Changed visibility", `${i.label} → ${label}`, "family"); store.save(); closeTop(); render(); toast(`Visibility: ${label}`); } }, [label])),
    el("p", { class: "fineprint" }, ["Changing visibility never shares past data retroactively."]),
  ]) });
}

// ============ NUDGE (trust envelope) ============
export function openNudge(n: LifeNudge, store: LifeStore, rerender: () => void) {
  ...
  openSheet({ title: n.title, body: el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      n.id === "n-italian" ? el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.confirmInfo("i-p2"); store.audit("Confirmed inference", "Priya may prefer Italian", "family"); closeTop(); rerender(); toast("Confirmed — I'll use it for the anniversary plan."); } }, ["Confirm this preference"]) : null,
      n.id === "n-car" ? el("button", { class: "btn btn-primary full", type: "button", onClick: () => { closeTop(); openCare(store, rerender); } }, ["Open the care journey"]) : null,
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { store.audit("Corrected nudge", `too personal: ${n.title}`, n.category); toast("Understood — I'll ease off."); closeTop(); } }, ["Too personal — don't suggest this again"]),
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { toast("I'll remind you later."); closeTop(); } }, ["Remind me later"]),
    ]),
...
// ============ CARE JOURNEY ============
export function openCare(store: LifeStore, rerender: () => void) {
  const wrap = el("div", {});
  const stageLabel: Record<string, string> = { before: "Before appointment", during: "During appointment", after: "After appointment" };
  function render() {
    ...
    wrap.replaceChildren(
      ...
      el("div", { class: "escalation" }, [
        ...
        el("div", { class: "escalation-row" }, escalation.map(([label, color], i) => el("span", { class: `esc-pill${i === 1 ? " active" : ""}`, style: i === 1 ? `background:${color}; color:#fff;` : `color:${color}; background:${color}1a;` }, [label as string]))),
      ...
      // stages
      ...(["before", "during", "after"] as const).map((stage) => el("div", { class: "care-stage" }, [
        el("div", { class: "care-stage-head" }, [stageLabel[stage]]),
        ...CARE_STEPS.filter((s) => s.stage === stage).map((s) => careStepRow(s, store, render)),
      ])),
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", style: `background:${PAL.green};`, onClick: () => { store.audit("Coordinated care prep", "Reports gathered, calendar held; cab & refill await approval", "health"); closeTop(); rerender(); toast("Prep coordinated — reports ready, 2 items await your approval."); } }, ["Coordinate the prep"]),
...
function careStepRow(s: CareStep, store: LifeStore, render: () => void): HTMLElement {
...
// ============ WELLBEING ============
export function openWellbeing(store: LifeStore, rerender: () => void) {
  const wrap = el("div", {});
  ...
  function render() {
    const parts: (HTMLElement | null)[] = [
      ...
      t.correlations ? el("div", { class: "corr-card" }, [
        ...
        el("div", { class: "info-actions" }, [
          ...
          el("button", { class: "chip-btn ghost", type: "button", onClick: () => { t.correlations = false; store.save(); render(); toast("Pattern insights off.") } }, ["Turn off"]),
        ]),
      ]) : null,
      // micro-intervention
      ...
      ...MICRO.map((mi) => el("div", { class: "micro-card" }, [
        ...
        el("div", { class: "info-actions" }, [
          el("button", { class: "chip-btn primary", type: "button", onClick: () => { store.audit("Added micro-intervention", mi.title, "health"); toast("Added to your day."); } }, ["Add to day"]),
      ...
      block("Health-data consent", el("div", { class: "consent-toggles" }, [
        toggleRow("Use wearable data", "Sleep & activity from your device", t.wearable, () => { t.wearable = !t.wearable; store.audit("Health consent", `wearable ${t.wearable}`, "health"); store.save(); render(); }),
        toggleRow("Show pattern insights", "Personal correlations, clearly labelled", t.correlations, () => { t.correlations = !t.correlations; store.save(); render(); }),
        toggleRow("Use my check-ins", "How you say you feel", t.checkins, () => { t.checkins = !t.checkins; store.save(); render(); }),
      ])),
    ...
    wrap.replaceChildren(...(parts.filter(Boolean) as HTMLElement[]));
  }
  function toggleRow(title: string, sub: string, on: boolean, onClick: () => void): HTMLElement {
...
// ============ HOUSEHOLD BASKET ============
export function openBasket(store: LifeStore, rerender: () => void) {
  const wrap = el("div", {});
  const invLabel: Record<string, string> = { confirmed: "Confirmed", predicted: "Predicted", user_reported: "You reported", device: "Smart-shelf" };
  function render() {
    ...
    wrap.replaceChildren(
      ...
      ...items.map((b) => el("div", { class: `basket-item${b.included ? "" : " excluded"}` }, [
        el("label", { class: "basket-check" }, [el("input", { type: "checkbox", checked: b.included, "aria-label": `Include ${b.name}`, onChange: (e: Event) => { b.included = (e.target as HTMLInputElement).checked; render(); } })]),
        ...
        el("button", { class: "chip-btn ghost", type: "button", "aria-label": `Still have ${b.name}`, onClick: () => { store.data.basketExcluded.push(b.id); store.save(); render(); toast("Marked still available — Buddy learns."); } }, ["Have it"]),
      ])),
      // store comparison
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.audit("Confirmed grocery purchase", `₹${total} · FreshMart`, "purchasing"); closeTop(); rerender(); toast(`Order confirmed · ₹${total}. Categorised to household budget.`); } }, [`Confirm purchase · ₹${total.toLocaleString("en-IN")}`]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { toast("Prediction paused."); } }, ["Pause predictions"]),
      ]),
...
// ============ ASSET / CAR ============
export function openAsset(store: LifeStore, rerender: () => void) {
  const a = CAR;
  openSheet({ title: a.name, body: el("div", { class: "sheet-body" }, [
    el("div", { class: "asset-stats" }, [
      kv("Service due", a.serviceDue), kv("Est. cost / yr", a.estAnnualCost),
      kv("Insurance", a.insurance), kv("Compliance", a.compliance),
      kv("Responsible", a.responsible), kv("Purchased", a.purchaseDate),
    ]),
    ...
    block("Authorised service slots", el("div", { class: "slot-list" }, a.serviceSlots.map((s) => el("div", { class: "slot-row" }, [
      ...
      el("button", { class: `chip-btn ${s.authorised ? "primary" : ""}`, type: "button", onClick: () => { closeTop(); openFinancial({ ...FINANCIAL_ACTION, action: `Book car service · ${s.name}`, amount: s.price, to: s.name, safeToSpendImpact: `−₹${s.price}` }, store, rerender); } }, ["Book"]),
    ])))),
...
// ============ OCCASION ============
export function openOccasion(store: LifeStore, rerender: () => void) {
  const o = OCCASION;
  const wrap = el("div", {});
  function render() {
    ...
    wrap.replaceChildren(
      ...
      block("Gift & plan ideas", el("div", { class: "idea-list" }, o.giftIdeas.map((g) => el("div", { class: "idea-row" }, [
        ...
        g.source === "inferred" && !italianConfirmed ? el("button", { class: "chip-btn", type: "button", onClick: () => openPreferenceSource(store, render) }, ["Inspect"]) : el("button", { class: "chip-btn primary", type: "button", onClick: () => { closeTop(); openFinancial({ ...FINANCIAL_ACTION, action: `Reserve · ${g.label}`, amount: 3500, to: "Restaurant", safeToSpendImpact: "−₹3,500" }, store, rerender); } }, ["Book"]),
      ])))),
...
function openPreferenceSource(store: LifeStore, render: () => void) {
  openSheet({ title: "Private preference source", body: el("div", { class: "sheet-body" }, [
    ...
    el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.confirmInfo("i-p2"); store.audit("Confirmed inference", "Italian preference for occasion", "family"); closeTop(); render(); toast("Confirmed — I'll use it privately."); } }, ["Confirm & use privately"]),
    el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { store.correctInfo("i-p2"); closeTop(); render(); toast("Won't use it."); } }, ["Don't use this"]),
    el("p", { class: "fineprint" }, ["Buddy shows the type of signal, not the message contents."]),
  ]) });
}

// ============ MONEY ============
export function openMoney(store: LifeStore, rerender: () => void) {
  let tab: "safe" | "forecast" | "scenario" = "safe";
  const wrap = el("div", {});
  function render() {
    const seg = el("div", { class: "seg", role: "radiogroup", "aria-label": "Money view" }, ([["safe", "Safe to spend"], ["forecast", "Forecast"], ["scenario", "Scenarios"]] as const).map(([k, label]) =>
      el("button", { class: `seg-btn${tab === k ? " active" : ""}`, role: "radio", "aria-checked": tab === k, type: "button", onClick: () => { tab = k; render(); } }, [label])));
    const body: (HTMLElement | null)[] = [seg];
    ...
    wrap.replaceChildren(...(body.filter(Boolean) as HTMLElement[]));
  }
  render();
  openSheet({ title: "Money", body: el("div", { class: "sheet-body" }, [wrap]) });
}

function safeToSpendView(): HTMLElement {
  ...
  return el("div", {}, [
    ...
    el("div", { class: "meta-grid" }, [
      kv("Accounts included", `${s.accountsIncluded.length}`), kv("Excluded", `${s.accountsExcluded.length}`),
      kv("Last sync", s.lastSync), kv("Pending txns", `${s.pendingTxns}`),
    ]),
    el("div", { class: "info-actions" }, [
...
function forecastView(): HTMLElement {
...
function scenarioView(store: LifeStore, rerender: () => void): HTMLElement {
  const s = SCENARIO;
  return el("div", {}, [
    el("div", { class: "scenario-q" }, [el("span", { class: "ic", html: ICONS.spark }), s.question]),
    el("div", { class: "meta-grid" }, [
      kv("Monthly impact", s.monthlyImpact), kv("Lowest balance", s.lowestBalance),
      kv("Savings goal", s.savingsImpact), kv("Safety buffer", s.bufferImpact),
    ]),
    ...
    el("div", { class: "info-actions" }, [
      el("button", { class: "chip-btn primary", type: "button", onClick: () => { store.audit("Saved scenario as goal", s.question, "banking"); toast("Saved as a savings goal."); } }, ["Save as goal"]),
...
// ============ HIGH-RISK FINANCIAL ACTION + STEP-UP ============
export function openFinancial(fa: import("./models.js").FinancialAction, store: LifeStore, rerender: () => void) {
  openSheet({ title: "Confirm financial action", body: el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "fin-rows" }, [
      kv("Action", fa.action), kv("Amount", `₹${fa.amount.toLocaleString("en-IN")}`),
      kv("To", fa.to), kv("From", fa.from), kv("When", fa.when), kv("Fees", fa.fees),
      kv("Cancellation", fa.cancellation), kv("Impact on safe-to-spend", fa.safeToSpendImpact),
...
function openStepUp(fa: import("./models.js").FinancialAction, store: LifeStore, rerender: () => void) {
  const inp = el("input", { type: "password", class: "stepup-input", inputmode: "numeric", maxlength: "6", placeholder: "••••••", "aria-label": "Step-up passcode (demo — type any 6 digits)" }) as HTMLInputElement;
...
function runFinancial(fa: import("./models.js").FinancialAction, store: LifeStore, rerender: () => void) {
  store.audit("Authorised financial action", `${fa.action} · ₹${fa.amount}`, "banking");
  document.querySelectorAll<HTMLElement>(".sheet-scrim .sheet-close").forEach((b) => b.click());
  setTimeout(() => {
    openSheet({ title: "Receipt", body: el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "fin-rows" }, [kv("Reference", `TXN-${Date.now().toString().slice(-6)}`), kv("To", fa.to), kv("From", fa.from), kv("When", new Date().toLocaleString("en-US", { hour: "numeric", minute: "2-digit", day: "numeric", month: "short" }))]),
      fa.cancellation.includes("Can't") ? el("p", { class: "fineprint" }, ["This action can't be reversed."]) : el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { toast("Cancelled."); closeTop(); } }, ["Cancel this action"]),
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => { closeTop(); rerender(); } }, ["Back to Life"]),
    ]) });
    rerender();
  }, 300);
}
...
// ============ SHARED WORKFLOW ============
export function openSharedWorkflow(store: LifeStore, rerender: () => void) {
  ...
  openSheet({ title: w.title, body: el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      ...
      el("button", { class: "btn btn-ghost full danger", type: "button", onClick: () => { store.audit("Revoked access", "Removed a participant", "shared"); toast("Access revoked."); } }, ["Revoke someone's access"]),
    ]),
...
function openSharePreview(store: LifeStore) {
  openSheet({ title: "Before you share", body: el("div", { class: "sheet-body" }, [
    ...
    el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.audit("Shared information", "Visit schedule → Ravi", "shared"); closeTop(); toast("Shared with Ravi — you can revoke anytime."); } }, ["Share this"]),
    el("p", { class: "fineprint" }, ["Only the exact item above is shared — nothing else."]),
  ]) });
}

// ============ PERMISSIONS ============
export function openPermissions(store: LifeStore, rerender: () => void) {
  const wrap = el("div", {});
    ...
    wrap.replaceChildren(
      ...
      ...PERMISSION_CATEGORIES.map((c) => {
        ...
        return el("div", { class: "perm-row" }, [
          ...
          el("select", { class: "perm-select", "aria-label": `Permission for ${c.label}`, onChange: (e: Event) => { store.data.permissions[c.id] = (e.target as HTMLSelectElement).value as any; store.audit("Changed permission", `${c.label} → ${PERMISSION_LABELS[(e.target as HTMLSelectElement).value]}`, c.id as any); store.save(); render(); } },
...
// ============ ACTIVITY / AUDIT ============
export function openActivity(store: LifeStore) {
  ...
  openSheet({ title: "Life activity & permissions", body: el("div", { class: "sheet-body" }, [
    ...
    el("button", { class: "btn btn-ghost sm danger full", type: "button", style: "margin-top:14px;", onClick: () => { store.reset(); closeTop(); toast("Life demo reset — memory cleared."); } }, ["Delete Buddy's Life memory (reset)"]),
  ]) });
}
