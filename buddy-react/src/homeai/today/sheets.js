...
import { estimateCompletion, energyRecommendations } from "./engines";
const CONF = {
...
function openStepPreview(opts) {
  ...
  function updateRun() {
    ...
    runBtn.textContent = active.length === 0 ? "Select at least one step" : unconfirmed.length > 0 ? `Confirm ${unconfirmed.length} action${unconfirmed.length > 1 ? "s" : ""} above` : `Confirm & apply ${active.length} step${active.length > 1 ? "s" : ""}`;
  }
  function render() {
      ...
      return el("div", { class: `preview-row risk-${s.riskLevel}` }, [
        el("label", { class: "preview-check" }, [
          el("input", {
            ...
            onChange: (e) => {
              const c = e.target;
              if (c.checked) selected.add(s.id);
              else {
        ...
        el("div", { class: "preview-main" }, [
          ...
          need && selected.has(s.id) ? el("label", { class: `confirm-line${permBlocked ? " blocked" : ""}` }, [
            el("input", {
              ...
              "aria-label": `Confirm ${s.description}`,
              onChange: (e) => {
                const c = e.target;
                if (c.checked) confirmed.add(s.id);
                else confirmed.delete(s.id);
                updateRun();
              }
            }),
            permBlocked ? el("span", {}, [`Needs the "${s.permissionRequired}" permission — grant it in Settings first.`]) : el("span", {}, [`Yes, I confirm this ${s.riskLevel}-risk action.`])
          ]) : null
        ])
      ]);
    }));
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    list,
    results,
    el("div", { class: "sheet-actions" }, [runBtn])
  ]);
  openSheet({ title: opts.title, body });
}
function openBrief(store, brief = MEETING_BRIEF) {
  let level = "glance";
  let playing = false;
  let cue = 0;
  let timer;
  ...
  function renderSeg() {
    seg.replaceChildren(...[["glance", "30-sec"], ["two", "2-min"], ["full", "Full context"]].map(([k, lbl]) => el("button", { class: `seg-btn${level === k ? " active" : ""}`, role: "radio", "aria-checked": level === k, type: "button", onClick: () => {
      level = k;
      render();
    } }, [lbl])));
  }
  ...
  function render() {
    ...
    if (level === "glance") {
      ...
      parts.push(
        ...
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Questions to ask"]), ...brief.recommendedQuestions.map((q) => el("div", { class: "q-row" }, [el("span", {}, [q]), el("button", { class: "chip-btn", type: "button", onClick: () => {
          store.data.savedQuestions.push(q);
          store.save();
          toast("Question saved to your companion.");
        } }, ["Save"])]))]),
    ...
    parts.push(
      el("div", { class: "brief-actions" }, [
        ...
        el("button", { class: "chip-btn", type: "button", onClick: () => {
          store.addFeedback(brief.meetingId, "correct_brief");
          toast("Thanks — tell me what was off and I'll adjust.");
        } }, ["Correct Buddy"]),
...
function openCompanion(store, onEnd) {
...
function openConsent(store, onStart) {
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        store.data.recordingConsent = true;
        store.save();
        store.audit({ trigger: "user", action: "Started meeting recording", sources: ["companion"], decision: "consented", result: "completed", undoStatus: "none", permissionUsed: "recording.consent" });
        document.querySelector(".sheet-scrim.open .sheet-close")?.click();
        onStart();
      } }, ["Start recording"]),
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "info-block" }, [
      ...
      ...followups.map((f) => el("label", { class: "followup-row" }, [
        el("input", { type: "checkbox", "aria-label": f.label, onChange: (e) => {
          const c = e.target;
          if (c.checked) picked.add(f.id);
...
function openDecomposition(store, onChange) {
  if (!store.data.taskClarified) {
    openClarify(store, () => openDecomposition(store, onChange));
    return;
  }
  const plan = buildTaskPlan(true);
  if (store.data.taskFirstPassDone) plan.steps.forEach((s) => {
    if (s.buddyCapability === "buddy_first_pass") {
      s.status = "buddy_done";
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    list,
    el("div", { class: "sheet-actions col" }, [
      store.data.taskFirstPassDone ? null : el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        store.data.taskFirstPassDone = true;
        store.save();
        store.audit({ trigger: "user", action: "Buddy first pass on API review", sources: ["confluence", "jira"], decision: "approved", result: "completed", undoStatus: "available", permissionUsed: null });
        document.querySelector(".sheet-scrim.open .sheet-close")?.click();
        openDecomposition(store, onChange);
        toast("Buddy completed the first pass on 4 steps.");
      } }, ["Let Buddy do the first pass"]),
...
function openClarify(store, onDone) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "info-block" }, [
      el("div", { class: "info-label" }, ["What should this review produce?"]),
      ...opts.map((o) => el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
        store.data.taskClarified = true;
        store.save();
        store.audit({ trigger: "user", action: `Clarified review output: ${o}`, sources: [], decision: "answered", result: "completed", undoStatus: "none", permissionUsed: null });
        document.querySelector(".sheet-scrim.open .sheet-close")?.click();
        onDone();
      } }, [o]))
    ]),
...
function openRecovery(plans, store, onApply) {
  let pick = plans[0].id;
  ...
  function render() {
    wrap.replaceChildren(
      ...
      el("label", { class: "protect-toggle" }, [
        el("input", { type: "checkbox", checked: protectPersonal, "aria-label": "Protect my family pickup above all", onChange: (e) => {
          protectPersonal = e.target.checked;
          render();
        } }),
      ...
      el("div", { class: "recovery-grid", role: "radiogroup", "aria-label": "Recovery options" }, plans.map((p) => {
        ...
        return el("button", {
          class: `res-card${pick === p.id ? " picked" : ""}${dimmed ? " dimmed" : ""}`,
          type: "button",
          role: "radio",
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
          ...
          document.querySelector(".sheet-scrim.open .sheet-close")?.click();
          openStepPreview({ title: `Apply: ${plan.name}`, steps: plan.steps, store, trigger: "overrun recovery", onDone: onApply });
        } }, ["Apply this plan"]),
        ...
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          store.data.overrunActive = false;
          store.save();
          onApply();
          document.querySelector(".sheet-scrim.open .sheet-close")?.click();
          toast("Kept your original plan.");
        } }, ["Ignore & restore plan"])
      ]),
...
function openEnergy(store, onChange) {
  const wrap = el("div", {});
  function render() {
    ...
    const parts = [
      ...
      el("div", { class: "energy-grid" }, states.map(([k, lbl]) => el("button", {
        class: `energy-btn${store.data.energy === k ? " active" : ""}`,
        type: "button",
        "aria-pressed": store.data.energy === k,
        onClick: () => {
          store.data.energy = k;
          store.save();
          store.addFeedback("energy", "checkin", k);
          render();
          onChange();
        }
      }, [lbl]))),
      ...
      el("div", { class: "info-block" }, [
        el("div", { class: "consent-row" }, [
          ...
          el("button", { class: `toggle sm${store.data.healthConsent ? " warn" : ""}`, type: "button", "aria-pressed": store.data.healthConsent, "aria-label": "Toggle health data", onClick: () => {
            store.data.healthConsent = !store.data.healthConsent;
            store.save();
            store.audit({ trigger: "user", action: store.data.healthConsent ? "Enabled health data" : "Disabled health data", sources: ["wellbeing"], decision: "consent", result: "completed", undoStatus: "none", permissionUsed: "health.consent" });
            render();
            onChange();
            toast(store.data.healthConsent ? "Health data on — private to you." : "Health data off and this session's read discarded.");
          } }, [store.data.healthConsent ? "On" : "Off"])
        ]),
...
function openCompletion(input, store) {
  ...
  function render() {
    ...
    const seg = el("div", { class: "seg", role: "radiogroup", "aria-label": "Scenario" }, [["current", "Current"], ["optimised", "Optimised"], ["delegated", "Delegated"], ["personal_first", "Family-first"]].map(([k, lbl]) => el("button", { class: `seg-btn${scenario === k ? " active" : ""}`, role: "radio", "aria-checked": scenario === k, type: "button", onClick: () => {
      scenario = k;
      render();
    } }, [lbl])));
    wrap.replaceChildren(
...
function openCompare(store, onApply) {
  const rows = [
  ...
  function render() {
    wrap.replaceChildren(
      el("div", { class: "seg", role: "radiogroup", "aria-label": "Timeline view" }, [["current", "Current"], ["optimised", "Optimised"]].map(([k, lbl]) => el("button", { class: `seg-btn${view === k ? " active" : ""}`, role: "radio", "aria-checked": view === k, type: "button", onClick: () => {
        view = k;
        render();
      } }, [lbl]))),
...
function openSuggestionWhy(s, store) {
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
      store.addFeedback(s.id, "no_recommend_type", s.type);
      toast("Noted — I'll ease off suggestions like this.");
    } }, ["Don't suggest this type"]),
