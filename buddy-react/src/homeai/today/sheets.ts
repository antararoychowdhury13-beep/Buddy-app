...
import { hasPermission, riskLabel } from "../engines/policy";
import { simulateStep } from "../engines/execution";
import { el, openSheet, toast, announce } from "../ui/dom";
import { ICONS } from "../ui/icons";
import type {
  ActionStep, CompletionEstimate, MeetingBrief, RecoveryPlan, ScheduleSuggestion, TaskPlan,
} from "./models";
import { MEETING_BRIEF, buildTaskPlan } from "./mock";
import type { TodayStore } from "./store";
import { estimateCompletion, energyRecommendations, type Scenario, type SimInput } from "./engines";
...
export function openStepPreview(opts: {
  title: string; steps: ActionStep[]; store: TodayStore; trigger: string;
  onDone?: () => void; staleSystems?: string[];
}) {
      ...
      return el("div", { class: `preview-row risk-${s.riskLevel}` }, [
        el("label", { class: "preview-check" }, [
          el("input", { type: "checkbox", checked: selected.has(s.id), "aria-label": `Include: ${s.description}`,
            onChange: (e: Event) => { const c = e.target as HTMLInputElement; if (c.checked) selected.add(s.id); else { selected.delete(s.id); confirmed.delete(s.id); } render(); } }),
        ]),
        el("div", { class: "preview-main" }, [
          ...
          need && selected.has(s.id) ? el("label", { class: `confirm-line${permBlocked ? " blocked" : ""}` }, [
            el("input", { type: "checkbox", disabled: permBlocked, "aria-label": `Confirm ${s.description}`,
              onChange: (e: Event) => { const c = e.target as HTMLInputElement; if (c.checked) confirmed.add(s.id); else confirmed.delete(s.id); updateRun(); } }),
  ...
  runBtn.addEventListener("click", async () => {
    ...
    for (const s of steps.filter((x) => selected.has(x.id))) {
      s.executionStatus = "executing"; render();
      ...
      s.result = res.message; s.undoAvailable = res.undoAvailable;
      ...
      const row = el("div", { class: `exec-row exec-${tone}` }, [
        ...
        el("div", { class: "exec-body" }, [
          ...
          res.undoAvailable ? el("button", { class: "btn btn-ghost sm", type: "button", onClick: (e: Event) => {
            s.executionStatus = "undone"; store.audit({ trigger: "user", action: `Undo: ${s.description}`, sources: [s.targetSystem], decision: "undo", result: "undone", undoStatus: "undone", permissionUsed: s.permissionRequired });
            (e.target as HTMLButtonElement).replaceWith(el("span", { class: "undone-tag" }, ["Undone"]));
            toast("Change undone."); announce("Change undone.");
...
export function openBrief(store: TodayStore, brief: MeetingBrief = MEETING_BRIEF) {
  let level: "glance" | "two" | "full" = "glance";
  let playing = false; let cue = 0; let timer: number | undefined;
  const wrap = el("div", {});

  const seg = el("div", { class: "seg", role: "radiogroup", "aria-label": "Brief depth" });
  function renderSeg() {
    seg.replaceChildren(...([["glance", "30-sec"], ["two", "2-min"], ["full", "Full context"]] as const).map(([k, lbl]) =>
      el("button", { class: `seg-btn${level === k ? " active" : ""}`, role: "radio", "aria-checked": level === k, type: "button", onClick: () => { level = k; render(); } }, [lbl])));
  }

  const transcriptBox = el("div", { class: "transcript", "aria-live": "polite" });
  function renderTranscript() {
    transcriptBox.replaceChildren(...brief.verbalTranscript.map((line, i) =>
      el("p", { class: `t-line${i === cue && playing ? " active" : ""}` }, [line.text])));
  }
  ...
  playBtn.addEventListener("click", () => {
    ...
    if (playing) {
      ...
      const advance = () => {
        renderTranscript();
        if (cue >= brief.verbalTranscript.length - 1) { playing = false; playBtn.querySelector("span:last-child")!.textContent = "Replay"; cue = 0; return; }
        cue++; timer = window.setTimeout(advance, 1400);
      };
      advance();
    } else { playBtn.querySelector("span:last-child")!.textContent = "Play verbal briefing"; if (timer) clearTimeout(timer); }
  });

  function sourceChips(): HTMLElement {
    return el("div", { class: "chip-row" }, brief.sources.map((s) =>
      el("button", { class: "src-chip", type: "button", onClick: () => openTodaySource(s) }, [s[0].toUpperCase() + s.slice(1)])));
  }
  ...
  function render() {
    ...
    if (level === "glance") {
      ...
      parts.push(
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Previous decisions"]), el("ul", { class: "bullet" }, brief.previousDecisions.map((d) => el("li", {}, [d])))]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Likely to come up"]), el("ul", { class: "bullet" }, brief.predictedRisks.map((r) => el("li", {}, [r])))]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Questions to ask"]), ...brief.recommendedQuestions.map((q) => el("div", { class: "q-row" }, [el("span", {}, [q]), el("button", { class: "chip-btn", type: "button", onClick: () => { store.data.savedQuestions.push(q); store.save(); toast("Question saved to your companion."); } }, ["Save"])]))]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Recommended position"]), el("p", {}, [brief.recommendedPosition])]),
      );
    } else {
      parts.push(
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Attendees"]), ...brief.attendees.map((a) => el("div", { class: "att-row" }, [el("div", { class: "att-name" }, [`${a.name} · ${a.role}`]), el("div", { class: "att-ctx" }, [a.context])]))]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Previous summary"]), el("p", {}, [brief.previousSummary])]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Unresolved actions"]), el("ul", { class: "bullet" }, brief.unresolvedActions.map((u) => el("li", {}, [u])))]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Relevant signals"]), ...brief.relevantSignals.map((s) => el("div", { class: "sig-row" }, [el("span", { class: "sig-src" }, [s.source]), el("span", {}, [s.note])]))]),
        el("div", { class: "info-block warn-block" }, [el("div", { class: "info-label" }, ["Missing information"]), el("p", {}, [brief.missingInformation])]),
        el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Data sources"]), sourceChips()]),
        el("p", { class: "muted sm" }, [brief.confidenceReason]),
      );
    }
    parts.push(
    ...
    renderTranscript();
    wrap.replaceChildren(seg, ...parts.filter(Boolean) as HTMLElement[]);
  }
  render();
  openSheet({ title: `Brief · ${brief.title}`, body: el("div", { class: "sheet-body" }, [wrap]) });
}

function addNote(store: TodayStore) {
  const input = el("input", { type: "text", class: "note-input", placeholder: "A private note for this meeting…", "aria-label": "Private note" });
  const body = el("div", { class: "sheet-body" }, [
    input,
    el("button", { class: "btn btn-primary full", type: "button", onClick: () => { const v = (input as HTMLInputElement).value.trim(); if (v) { store.data.privateNotes.push(v); store.save(); toast("Private note saved."); } (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); } }, ["Save note"]),
...
function openTodaySource(id: string) {
...
export function openCompanion(store: TodayStore, onEnd: () => void) {
  if (!store.data.recordingConsent) { openConsent(store, () => openCompanion(store, onEnd)); return; }
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => { (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); openPostMeeting(store, onEnd); } }, ["Simulate meeting end"]),
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { store.data.recordingConsent = false; store.save(); toast("Recording stopped."); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); } }, ["Stop recording"]),
    ]),
  ]);
  openSheet({ title: "Meeting Companion", body });
}

function openConsent(store: TodayStore, onStart: () => void) {
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.data.recordingConsent = true; store.save(); store.audit({ trigger: "user", action: "Started meeting recording", sources: ["companion"], decision: "consented", result: "completed", undoStatus: "none", permissionUsed: "recording.consent" }); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); onStart(); } }, ["Start recording"]),
      el("button", { class: "btn btn-ghost full", type: "button", onClick: () => (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click() }, ["Cancel"]),
    ]),
...
function openPostMeeting(store: TodayStore, onEnd: () => void) {
  const confirmed = ["Launch date held at 24 Jul"];
  const proposed = ["Add a 2-day internal buffer (needs Rohan's nod)"];
  const followups = [
    { id: "f1", label: "Send Sara the revised API ETA note", risk: "medium" as const, perm: "email.send" },
    { id: "f2", label: "Post the decision summary to #powervc-launch", risk: "medium" as const, perm: "messages.send_external" },
  ];
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "info-block" }, [
      ...
      ...followups.map((f) => el("label", { class: "followup-row" }, [
        el("input", { type: "checkbox", "aria-label": f.label, onChange: (e: Event) => { const c = e.target as HTMLInputElement; if (c.checked) picked.add(f.id); else picked.delete(f.id); } }),
    ...
    el("div", { class: "sheet-actions col" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        ...
        if (chosen.length === 0) { toast("Nothing selected — no messages sent."); return; }
        const steps: ActionStep[] = chosen.map((f) => ({ id: f.id, description: f.label, targetSystem: f.id === "f2" ? "Teams" : "Email", riskLevel: f.risk, reversible: false, permissionRequired: f.perm, preview: "This message is sent on your behalf. Needs your explicit approval.", executionStatus: "suggested", undoAvailable: false }));
...
// ---- task decomposition ----
export function openDecomposition(store: TodayStore, onChange: () => void) {
  if (!store.data.taskClarified) { openClarify(store, () => openDecomposition(store, onChange)); return; }
  const plan = buildTaskPlan(true);
  if (store.data.taskFirstPassDone) plan.steps.forEach((s) => { if (s.buddyCapability === "buddy_first_pass") { s.status = "buddy_done"; s.output = "Buddy drafted this — review when ready."; } });
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    list,
    el("div", { class: "sheet-actions col" }, [
      store.data.taskFirstPassDone ? null : el("button", { class: "btn btn-primary full", type: "button", onClick: () => { store.data.taskFirstPassDone = true; store.save(); store.audit({ trigger: "user", action: "Buddy first pass on API review", sources: ["confluence", "jira"], decision: "approved", result: "completed", undoStatus: "available", permissionUsed: null }); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); openDecomposition(store, onChange); toast("Buddy completed the first pass on 4 steps."); } }, ["Let Buddy do the first pass"]),
      el("button", { class: "btn btn-tonal full", type: "button", onClick: () => {
        const steps: ActionStep[] = [{ id: "focus-conv", description: "Convert the remaining review steps into a 3:00 PM focus block", targetSystem: "Calendar", riskLevel: "low", reversible: true, permissionRequired: "calendar.block_focus", preview: "A focus block for the API review appears at 3:00 PM.", executionStatus: "suggested", undoAvailable: true }];
        (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click();
...
function openClarify(store: TodayStore, onDone: () => void) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["What should this review produce?"]),
      ...opts.map((o) => el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { store.data.taskClarified = true; store.save(); store.audit({ trigger: "user", action: `Clarified review output: ${o}`, sources: [], decision: "answered", result: "completed", undoStatus: "none", permissionUsed: null }); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); onDone(); } }, [o]))]),
...
// ---- recovery plans ----
export function openRecovery(plans: RecoveryPlan[], store: TodayStore, onApply: () => void) {
  let pick = plans[0].id; let protectPersonal = false;
  const wrap = el("div", {});
  function render() {
    wrap.replaceChildren(
      el("p", { class: "muted", style: "margin-bottom:12px;" }, ["The 11 AM sync overran 25 minutes. Here are three ways to recover — compare and choose."]),
      el("label", { class: "protect-toggle" }, [
        el("input", { type: "checkbox", checked: protectPersonal, "aria-label": "Protect my family pickup above all", onChange: (e: Event) => { protectPersonal = (e.target as HTMLInputElement).checked; render(); } }),
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => { const plan = plans.find((p) => p.id === pick)!; (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); openStepPreview({ title: `Apply: ${plan.name}`, steps: plan.steps, store, trigger: "overrun recovery", onDone: onApply }); } }, ["Apply this plan"]),
        ...
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { store.data.overrunActive = false; store.save(); onApply(); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); toast("Kept your original plan."); } }, ["Ignore & restore plan"]),
      ]),
...
// ---- energy check-in ----
export function openEnergy(store: TodayStore, onChange: () => void) {
  const wrap = el("div", {});
  function render() {
    const states: [import("./models.js").EnergyState, string][] = [["energised", "Energised"], ["ok", "Okay"], ["low", "Low energy"], ["over", "Overloaded"], ["unspecified", "Prefer not to say"]];
    const parts: (HTMLElement | null)[] = [
...
// ---- completion estimate ----
export function openCompletion(input: SimInput, store: TodayStore) {
  let scenario: Scenario = "current";
  const wrap = el("div", {});
  const BAND_META: Record<string, { color: string; bg: string; pct: string }> = {
  ...
  function render() {
    const est: CompletionEstimate = estimateCompletion(scenario, input, store.data.healthConsent, store.data.energy);
    const pct = Math.round(est.internalProbability * 100);
    const bm = BAND_META[est.band];
    const seg = el("div", { class: "seg", role: "radiogroup", "aria-label": "Scenario" }, ([["current", "Current"], ["optimised", "Optimised"], ["delegated", "Delegated"], ["personal_first", "Family-first"]] as [Scenario, string][]).map(([k, lbl]) =>
      el("button", { class: `seg-btn${scenario === k ? " active" : ""}`, role: "radio", "aria-checked": scenario === k, type: "button", onClick: () => { scenario = k; render(); } }, [lbl])));
    wrap.replaceChildren(
      seg,
      el("div", { class: "est-hero", style: `background:${bm.bg};` }, [
      ...
      el("div", { class: "conf-row", style: "margin:10px 0;" }, [confBadge(est.confidence), el("span", { class: "muted sm" }, [`Calculated ${est.calculatedAt}`])]),
      el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["What's helping"]), el("ul", { class: "bullet good-bullet" }, est.positiveFactors.map((f) => el("li", {}, [f])))]),
      el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["What's working against it"]), el("ul", { class: "bullet bad-bullet" }, est.negativeFactors.map((f) => el("li", {}, [f])))]),
      el("div", { class: "info-block" }, [el("div", { class: "info-label" }, ["Assumptions"]), el("ul", { class: "bullet" }, est.assumptions.map((f) => el("li", {}, [f])))]),
...
export function openCompare(store: TodayStore, onApply: () => void) {
  const rows = [
  ...
  let view: "current" | "optimised" = "optimised";
  const wrap = el("div", {});
  function render() {
    wrap.replaceChildren(
      el("div", { class: "seg", role: "radiogroup", "aria-label": "Timeline view" }, ([["current", "Current"], ["optimised", "Optimised"]] as const).map(([k, lbl]) =>
        el("button", { class: `seg-btn${view === k ? " active" : ""}`, role: "radio", "aria-checked": view === k, type: "button", onClick: () => { view = k; render(); } }, [lbl]))),
      el("div", { class: "cmp-table", role: "table" }, [
      ...
      el("p", { class: "fineprint" }, ["Estimates, not guarantees. Two changes affect other people and will ask before applying."]),
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => { (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); onApply(); } }, ["Apply safe optimisations"]),
      ]),
    );
  }
  render();
...
export function openSuggestionWhy(s: ScheduleSuggestion, store: TodayStore) {
  const body = el("div", { class: "sheet-body" }, [
