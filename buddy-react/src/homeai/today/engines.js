...
function simulateDay(input) {
  ...
  const sum = (type) => t.filter((b) => b.type === type).reduce((a, b) => a + (b.endMins - b.startMins), 0);
  ...
  if (input.appliedSuggestions.has("s-focus")) focusMins += 0;
  if (input.appliedSuggestions.has("s-buffer")) bufferMins += 10;
  if (input.appliedSuggestions.has("s-vendor")) {
  }
  let switches = 0;
  ...
  const expectedFinishMins = dayEndMins + overrunPush - optimisedPull + 30;
  ...
  const personalAtRisk = input.overrunActive && !input.appliedSuggestions.has("s-focus") && !input.appliedSuggestions.has("s-vendor");
  return {
    meetingMins,
    focusMins,
    travelMins,
...
function estimateCompletion(scenario, input, healthConsent, energy) {
  const m = simulateDay(input);
  const base = { current: 0.58, optimised: 0.82, delegated: 0.88, personal_first: 0.72 };
  let p = base[scenario];
  const positive = [];
  const negative = [];
  const assumptions = ["The 3 due tasks need ~80 focused minutes total", "No new urgent work lands this afternoon"];
  if (scenario === "optimised") {
    positive.push("Protects a clean 2h 15m focus block");
    positive.push("Buffer added after the overrun-prone sync");
  }
  if (scenario === "delegated") {
    positive.push("Buddy drafts the sprint summary, removing ~20 min");
    positive.push("Focus block stays fully protected");
  }
  if (scenario === "personal_first") {
    positive.push("Leaving on time for pickup is guaranteed");
    negative.push("Slightly less focus time before 5:15 PM");
  }
  if (scenario === "current") {
    negative.push("Focus time is fragmented across the day");
    negative.push("No buffer for the overrun-prone 11 AM sync");
  }
  if (input.overrunActive) {
    p -= 0.12;
    negative.push("The 11 AM sync overran by 25 min, eating into the afternoon");
  }
  if (m.personalAtRisk) {
    negative.push("Family pickup at 5:15 PM is currently at risk");
  }
  ...
  if (healthConsent) {
    healthUsed = true;
    dataUsed.push("wellbeing");
    if (energy === "low" || energy === "over") {
      p -= 0.08;
      negative.push("You checked in low on energy, so deep work may be slower");
    } else if (energy === "energised") {
      p += 0.05;
      positive.push("You're feeling energised — good for deep work");
    }
  } else if (energy === "low" || energy === "over") {
    p -= 0.05;
    negative.push("You mentioned low energy (no health data used — just your check-in)");
  }
  p = Math.min(0.95, Math.max(0.15, p));
  const band = p >= 0.82 ? "very likely" : p >= 0.62 ? "likely" : p >= 0.4 ? "uncertain" : "unlikely";
  const confidence = input.calendarStale ? "low" : p >= 0.6 ? "high" : "medium";
  return {
    band,
    internalProbability: Math.round(p * 100) / 100,
    confidence,
    positiveFactors: positive,
    negativeFactors: negative,
    assumptions,
    dataUsed,
    healthDataUsed: healthUsed,
    calculatedAt: (/* @__PURE__ */ new Date()).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }),
    disclaimer: "This is planning guidance, not a performance score or guarantee. It's never shared with your manager."
  };
}
function energyRecommendations(energy) {
  switch (energy) {
    case "energised":
      return [
