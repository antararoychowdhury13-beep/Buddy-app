...
import { buildRecommendations, buildSummary, CATEGORY_LABELS } from "../engines/planner";
...
class HomeApp {
  ...
  modeBar(mode, reason, auto) {
    const chips = ALL_MODES.map((m) => el("button", {
      class: `mode-chip${m === mode ? " active" : ""}`,
      type: "button",
      role: "radio",
      "aria-checked": m === mode,
      onClick: () => {
        this.store.setMode(m);
        announce(`Home mode: ${MODE_CONFIG[m].label}`);
        this.render();
      }
    }, [MODE_CONFIG[m].label]));
    return el("section", { class: "mode-bar", "aria-label": "Home mode" }, [
      el("div", { class: "mode-top" }, [
        ...
        auto ? el("button", { class: "mode-link", type: "button", onClick: () => {
          this.store.setMode(mode);
          this.render();
        } }, ["Keep this layout"]) : el("button", { class: "mode-link", type: "button", onClick: () => {
  ...
  dailyPlan(mode) {
    ...
    const visible = this.recs.filter((r) => r.status === "suggested" || r.status === "accepted").filter((r) => !this.store.data.planEdits[r.id]?.disabled).filter((r) => !cfg.hidden.includes(r.category));
    const byCat = cfg.order.map((cat) => ({ cat, items: visible.filter((r) => r.category === cat) })).filter((g) => g.items.length > 0);
    ...
    return el("section", { class: "card plan-card", "aria-label": "Daily Operating Plan" }, [
      ...
      el("div", { class: "plan-stats" }, stats.map(([k, v, s]) => el("div", { class: "pstat" }, [
        el("div", { class: "pstat-v" }, [v]),
        el("div", { class: "pstat-k" }, [k]),
        el("div", { class: "pstat-s" }, [s])
      ]))),
  ...
  priorities() {
    const items = rankPriorities({
      recommendations: this.recs,
      conflicts: this.conflicts,
      prefs: this.store.data.prefs,
      manualOrder: this.store.data.manualOrder
      ...
      el("div", { class: "conf-body" }, [
        ...
        el("div", { class: "conf-actions" }, [
          ...
          el("button", { class: "chip-btn", type: "button", onClick: () => {
            c.status = "deferred";
            this.store.audit("Deferred conflict", c.title, "deferred", true);
            this.render();
            toast("Decide later.");
          } }, ["Decide later"])
        ])
  ...
  acceptEntirePlan() {
    ...
    openPlanPreview(active, this.store, () => {
      this.afterExecution();
    });
  }
  acceptItem(r) {
      ...
      return el("div", { class: `edit-row${e.disabled ? " disabled" : ""}` }, [
        el("div", { class: "edit-main" }, [
          ...
          el("div", { class: "edit-controls" }, [
            el("label", { class: "edit-field" }, ["Time ", el("input", { type: "text", value: e.time ?? r.defaultTime ?? "", "aria-label": `Time for ${r.title}`, onChange: (ev) => {
              this.store.data.planEdits[r.id] = { ...e, time: ev.target.value };
              this.store.save();
            } })]),
            r.buddyCanHandle ? el("button", { class: "toggle", type: "button", "aria-pressed": handling === "buddy", onClick: () => {
              this.store.data.planEdits[r.id] = { ...e, handling: handling === "buddy" ? "remind" : "buddy" };
              this.store.save();
              rebuild();
            } }, [handling === "buddy" ? "Buddy handles" : "Remind me"]) : el("span", { class: "muted sm" }, ["Remind me"])
          ]),
          el("input", { type: "text", class: "edit-instruction", placeholder: "Add an instruction for Buddy…", value: e.instruction ?? "", "aria-label": `Instruction for ${r.title}`, onChange: (ev) => {
            this.store.data.planEdits[r.id] = { ...e, instruction: ev.target.value };
            this.store.save();
          } })
        ]),
        el("div", { class: "edit-side" }, [
          el("button", { class: "toggle sm", type: "button", "aria-pressed": !e.disabled, onClick: () => {
            this.store.data.planEdits[r.id] = { ...e, disabled: !e.disabled };
            this.store.save();
            rebuild();
          } }, [e.disabled ? "Off" : "On"])
        ])
    ...
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "sheet-actions col" }, [
        ...
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          for (const r of editable) delete this.store.data.planEdits[r.id];
          this.store.save();
          rebuild();
          toast("Restored Buddy's recommendations.");
        } }, ["Restore Buddy's plan"])
      ])
    ]);
    ...
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "src-list" }, [
        ...
        return el("div", { class: "src-row" }, [
          ...
          el("button", { class: `toggle sm${stale ? " warn" : ""}`, type: "button", "aria-pressed": stale, "aria-label": `${id} data ${stale ? "stale" : "fresh"}`, onClick: () => {
            this.store.toggleStale(id, !stale);
            this.render();
            toast(stale ? "Marked fresh." : "Marked stale — confidence will drop where used.");
            document.querySelector(".sheet-scrim.open .sheet-close")?.click();
          } }, [stale ? "Stale" : "Fresh"])
        ]);
      }))
  ...
  resetDemo() {
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full danger", type: "button", onClick: () => {
          this.store.reset();
          this.recs = buildRecommendations(this.ctx);
          this.conflicts = detectConflicts(this.ctx);
          this.render();
          document.querySelector(".sheet-scrim.open .sheet-close")?.click();
          toast("Demo reset.");
        } }, ["Reset everything"]),
