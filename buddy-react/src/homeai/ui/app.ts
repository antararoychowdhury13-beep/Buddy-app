/** Main Home renderer: greeting, mode bar, Daily Operating Plan, ranked
 * priorities, conflicts, feedback, and edit/compare/preview flows. */
import type { HomeContext, HomeMode, PlanCategory, Recommendation } from "../models";
import { fmtMins } from "../mock";
import { buildRecommendations, buildSummary, CATEGORY_LABELS, type PlanState } from "../engines/planner";
import { detectConflicts } from "../engines/conflicts";
import { rankPriorities, TIER_META } from "../engines/ranking";
import { ALL_MODES, MODE_CONFIG, currentContext, selectMode } from "../engines/modes";
import { Store } from "../store";
import { announce, clear, el, openSheet, toast } from "./dom";
import { DOMAIN_META, ICONS } from "./icons";
import {
  openComparePlans, openConflictSheet, openPlanPreview, openPriorityActions,
  openSourceSheet, openWhySheet, setSheetDataSources,
} from "./sheets";
...
export class HomeApp {
  ...
  private applyStatuses() {
    for (const r of this.recs) {
      ...
      if (s) r.status = s as any;
    }
  }

  private activeMode(): { mode: HomeMode; reason: string; auto: boolean } {
  ...
  private modeBar(mode: HomeMode, reason: string, auto: boolean): HTMLElement {
  ...
  private dailyPlan(mode: HomeMode): HTMLElement {
  ...
  private capForStress(groups: { cat: PlanCategory; items: Recommendation[] }[], max: number) {
    const out: { cat: PlanCategory; items: Recommendation[] }[] = [];
    let count = 0;
  ...
  private recCard(r: Recommendation): HTMLElement {
  ...
  private footerControls(): HTMLElement {
  ...
  private rejectItem(r: Recommendation) {
    r.status = "rejected";
  ...
  private compareItem(r: Recommendation) {
  ...
  private afterExecution() {
    ...
    setTimeout(() => {
      const body = el("div", { class: "sheet-body" }, [
        ...
        ...[
          { k: "helpful" as const, label: "Yes" },
          { k: "not_helpful" as const, label: "Not really" },
          { k: "wrong_time" as const, label: "Wrong time" },
  ...
  private editPlan() {
      ...
      return el("div", { class: `edit-row${e.disabled ? " disabled" : ""}` }, [
        el("div", { class: "edit-main" }, [
          ...
          el("div", { class: "edit-controls" }, [
            el("label", { class: "edit-field" }, ["Time ", el("input", { type: "text", value: e.time ?? r.defaultTime ?? "", "aria-label": `Time for ${r.title}`, onChange: (ev: Event) => { this.store.data.planEdits[r.id] = { ...e, time: (ev.target as HTMLInputElement).value }; this.store.save(); } })]),
          ...
          el("input", { type: "text", class: "edit-instruction", placeholder: "Add an instruction for Buddy…", value: e.instruction ?? "", "aria-label": `Instruction for ${r.title}`, onChange: (ev: Event) => { this.store.data.planEdits[r.id] = { ...e, instruction: (ev.target as HTMLInputElement).value }; this.store.save(); } }),
        ]),
    ...
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full", type: "button", onClick: () => { this.render(); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); toast("Plan updated."); } }, ["Save changes"]),
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { for (const r of editable) delete this.store.data.planEdits[r.id]; this.store.save(); rebuild(); toast("Restored Buddy's recommendations."); } }, ["Restore Buddy's plan"]),
      ]),
    ]);
    ...
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "src-list" }, [
        ...
        return el("div", { class: "src-row" }, [
          ...
          el("button", { class: `toggle sm${stale ? " warn" : ""}`, type: "button", "aria-pressed": stale, "aria-label": `${id} data ${stale ? "stale" : "fresh"}`, onClick: () => { this.store.toggleStale(id, !stale); this.render(); toast(stale ? "Marked fresh." : "Marked stale — confidence will drop where used."); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); } }, [stale ? "Stale" : "Fresh"]),
        ]);
      })),
    ]);
  ...
  private resetDemo() {
    const body = el("div", { class: "sheet-body" }, [
      ...
      el("div", { class: "sheet-actions col" }, [
        el("button", { class: "btn btn-primary full danger", type: "button", onClick: () => { this.store.reset(); this.recs = buildRecommendations(this.ctx); this.conflicts = detectConflicts(this.ctx); this.render(); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); toast("Demo reset."); } }, ["Reset everything"]),
