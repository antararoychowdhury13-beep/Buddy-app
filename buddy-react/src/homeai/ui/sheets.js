...
let DATA_SOURCES = [];
function setSheetDataSources(ds) {
  DATA_SOURCES = ds;
}
function staleSystems(store) {
...
function sourceChip(sourceId, store) {
...
function openSourceSheet(sourceId, store) {
  const src = DATA_SOURCES.find((s) => s.id === sourceId);
  if (!src) return;
...
function openWhySheet(rec, store) {
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
      store.addFeedback({ subjectId: rec.id, kind: "not_important", note: rec.domain });
      toast("Thanks — Buddy will weight this lower.");
    } }, ["Correct Buddy — this isn't important to me"]),
    el("p", { class: "fineprint" }, ["Nothing changes until you confirm."])
  ]);
  openSheet({ title: "Why now?", body });
}
function openPlanPreview(recs, store, onDone) {
      ...
      return el("div", { class: `preview-row risk-${step.riskLevel}` }, [
        el("label", { class: "preview-check" }, [
          el("input", {
            type: "checkbox",
            checked: isSel,
            "aria-label": `Include: ${step.description}`,
            onChange: (ev) => {
              const c = ev.target;
              if (c.checked) selected.add(step.id);
              else {
                selected.delete(step.id);
                confirmed.delete(step.id);
              }
              renderRows();
            }
          })
        ...
        el("div", { class: "preview-main" }, [
          ...
          need && isSel ? el("label", { class: `confirm-line${permBlocked ? " blocked" : ""}` }, [
            el("input", {
              ...
              "aria-label": `Explicit confirmation for ${step.description}`,
              onChange: (ev) => {
                const c = ev.target;
                if (c.checked) confirmed.add(step.id);
                else confirmed.delete(step.id);
                updateRun();
              }
            }),
            permBlocked ? el("span", {}, [`Needs the "${step.permissionRequired}" permission — grant it in Settings first.`]) : el("span", {}, [`Yes, I confirm this ${step.riskLevel}-risk action.`])
          ]) : null
        ])
      ]);
    }));
  ...
  function renderResult(rec, step, msg, warning, undoable) {
    ...
    const row = el("div", { class: `exec-row exec-${tone}` }, [
      ...
      el("div", { class: "exec-body" }, [
        ...
        undoable ? el("button", { class: "btn btn-ghost sm", type: "button", onClick: (ev) => {
          step.executionStatus = "undone";
          store.audit(`Undo: ${step.description}`, step.targetSystem, "undone", false);
          store.addFeedback({ subjectId: rec.id, kind: "undone", note: rec.domain });
          ev.target.closest(".exec-row")?.classList.add("undone-row");
          ev.target.replaceWith(el("span", { class: "undone-tag" }, ["Undone"]));
...
function openUndoLearnSheet(rec, store) {
  const body = el("div", { class: "sheet-body" }, [
    el("p", { style: "margin-bottom:14px;" }, ["You undid this change. What should Buddy learn?"]),
    ...[
      { k: "always_ask", label: "Always ask me first" },
      { k: "never_move_type", label: "Never move this type of item" },
      { k: "one_time_exception", label: "This was a one-time exception" }
...
function openComparePlans(store, onChoose) {
  const rows = comparePlans();
  let choice = store.data.planChoice;
  ...
  function renderSeg() {
    seg.replaceChildren(
      el("button", { class: `seg-btn${choice === "recommended" ? " active" : ""}`, role: "radio", "aria-checked": choice === "recommended", type: "button", onClick: () => {
        choice = "recommended";
        renderSeg();
      } }, ["Recommended"]),
      el("button", { class: `seg-btn${choice === "alternative" ? " active" : ""}`, role: "radio", "aria-checked": choice === "alternative", type: "button", onClick: () => {
        choice = "alternative";
        renderSeg();
      } }, ["Alternative"])
    );
  ...
  const body = el("div", { class: "sheet-body" }, [
    ...
    el("div", { class: "sheet-actions" }, [
      el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
        store.data.planChoice = choice;
        store.save();
        store.audit("Selected plan", choice, "chosen", true);
        onChoose(choice);
        document.querySelector(".sheet-scrim.open .sheet-close")?.click();
...
function openConflictSheet(c, store, onResolved) {
  ...
  const cm = CONF_META[conf];
  let picked = c.chosen ?? "A";
  function optionCard(o) {
    const isRec = o.id === "A";
    return el("button", {
      class: `res-card${picked === o.id ? " picked" : ""}`,
      type: "button",
      role: "radio",
  ...
  function render() {
    ...
    wrap.replaceChildren(
      ...
      el("div", { class: "sheet-actions col" }, [
        ...
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
          c.status = "deferred";
          store.audit("Deferred conflict", c.title, "deferred", true);
          onResolved();
          document.querySelector(".sheet-scrim.open .sheet-close")?.click();
          toast("Decide later — Buddy will keep an eye on it.");
        } }, ["Decide later"])
      ]),
      el("p", { class: "fineprint" }, ["Nothing changes until you confirm."])
    );
  }
  async function resolve(needsConfirm) {
    const chosen = picked === "A" ? c.recommended : c.alternative;
    ...
    if (needsConfirm) {
      ...
      const confirmBody = el("div", { class: "sheet-body" }, [
        ...
        permBlocked ? el("p", { class: "warn-block" }, [`This needs the "${chosen.permissionRequired}" permission, which isn't granted. Grant it in Settings — Buddy won't act without it.`]) : el("button", { class: "btn btn-primary full", type: "button", onClick: () => {
          document.querySelectorAll(".sheet-scrim.open .sheet-close")[1]?.click();
          proceed();
        } }, [`Confirm this ${chosen.riskLevel}-risk action`]),
  ...
  async function runResolution(chosen) {
    c.chosen = chosen.id;
    c.status = "resolved";
    ...
    onResolved();
    document.querySelectorAll(".sheet-scrim.open .sheet-close").forEach((b) => b.click());
...
function openConflictFeedback(c, store) {
  const body = el("div", { class: "sheet-body" }, [
    el("p", { style: "margin-bottom:14px;" }, ["Was this the right call?"]),
    ...[
      { k: "helpful", label: "Yes, good catch" },
      { k: "not_helpful", label: "Not really" },
      { k: "wrong_time", label: "Flagged too late" }
    ].map((o) => el("button", { class: "btn btn-ghost full", type: "button", onClick: () => {
...
function openPriorityActions(item, store, onChange) {
  const actions = [
    { label: "Move up", run: () => {
      store.data.manualOrder[item.refId] = (store.data.manualOrder[item.refId] ?? item.score) + 1;
      store.addFeedback({ subjectId: item.refId, kind: "rank_up", note: item.domain });
      store.save();
    } },
    { label: "Move down", run: () => {
      store.data.manualOrder[item.refId] = (store.data.manualOrder[item.refId] ?? item.score) - 1;
      store.addFeedback({ subjectId: item.refId, kind: "rank_down", note: item.domain });
      store.save();
    } },
    { label: "Not important to me", run: () => {
      store.addFeedback({ subjectId: item.refId, kind: "not_important", note: item.domain });
      store.data.recStatuses[item.refId] = "rejected";
      store.save();
    } },
    { label: "Snooze for 2 hours", run: () => {
      store.data.snoozed.push(item.refId);
      store.save();
    } },
    { label: "Mark complete", run: () => {
      store.data.recStatuses[item.refId] = "done";
      store.addFeedback({ subjectId: item.refId, kind: "accepted", note: item.domain });
      store.save();
    } }
  ];
  if (item.buddyCanHandle) actions.splice(2, 0, { label: "Delegate to Buddy", run: () => {
    store.data.recStatuses[item.refId] = "accepted";
    store.save();
  } });
  const body = el("div", { class: "sheet-body" }, [
