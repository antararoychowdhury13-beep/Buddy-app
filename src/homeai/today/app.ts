/** Today renderer: day-health, schedule-optimisation card, adaptive timeline,
 * completion snapshot, meeting/task entry points, demo controls, audit. */
import { clear, el, openSheet, toast, announce } from "../ui/dom.js";
import { ICONS } from "../ui/icons.js";
import type { ScheduleSuggestion, TimelineBlock } from "./models.js";
import { ADAPTERS, buildRecoveryPlans, buildSuggestions, buildTimeline, fmtDur } from "./mock.js";
import { estimateCompletion, simulateDay, type SimInput } from "./engines.js";
import { TodayStore } from "./store.js";
import {
  openBrief, openCompanion, openCompare, openCompletion, openDecomposition,
  openEnergy, openRecovery, openStepPreview, openSuggestionWhy,
} from "./sheets.js";
...
const FLEX_LABEL: Record<string, string> = { fixed: "Fixed", flexible: "Flexible", movable: "Movable" };

export class TodayApp {
  store = new TodayStore();
  root: HTMLElement;
  timeline: TimelineBlock[] = buildTimeline();
  suggestions: ScheduleSuggestion[] = buildSuggestions();
  ...
  private staleBanner(): HTMLElement | null {
  ...
  private optimisationCard(): HTMLElement {
  ...
  private optStat(v: string, k: string): HTMLElement {
    return el("div", { class: "pstat" }, [el("div", { class: "pstat-v" }, [v]), el("div", { class: "pstat-k" }, [k])]);
  }

  private completionSnapshot(): HTMLElement {
    const m = simulateDay(this.simInput());
    const stats: [string, string][] = [
      [`${fmtDur(m.focusMins)}`, "Focus"], [`${fmtDur(m.meetingMins)}`, "Meetings"],
      [`${m.contextSwitches}`, "Switches"], [m.expectedFinish, "Finish"],
    ];
  ...
  private nextMeeting(): HTMLElement {
  ...
  private timelineCard(): HTMLElement {
    const legend = el("div", { class: "legend" }, ["meeting", "focus", "personal", "buffer", "travel"].map((t) =>
      el("span", { class: "legend-item" }, [el("span", { class: "legend-dot", style: `background:${TYPE_META[t].bg}; color:${TYPE_META[t].color};`, "aria-hidden": "true", html: TYPE_META[t].icon }), TYPE_META[t].label])));
  ...
  private energyCard(): HTMLElement {
  ...
  private demoControls(): HTMLElement {
    const btn = (label: string, run: () => void) => el("button", { class: "demo-btn", type: "button", onClick: run }, [label]);
    return el("details", { class: "demo-panel" }, [
      el("summary", {}, [el("span", { class: "ic", html: ICONS.sliders }), "Demo controls"]),
      el("div", { class: "demo-grid" }, [
        btn("Trigger 25-min overrun", () => { this.store.data.overrunActive = true; this.store.save(); this.store.audit({ trigger: "demo", action: "Stakeholder sync overran 25 min", sources: ["calendar"], decision: "system", result: "completed", undoStatus: "none", permissionUsed: null }); announce("Day is replanning after a 25-minute overrun."); this.render(); openRecovery(buildRecoveryPlans(), this.store, () => this.render()); }),
        btn("Add urgent task", () => { toast("Urgent issue added — completion odds recalculated."); this.render(); }),
        btn("Traffic delay", () => { toast("Traffic worsened — leave 10 min earlier flagged on your commute."); }),
        btn("Mark task complete", () => { toast("Nice — one due task cleared. Odds improved."); this.render(); }),
        btn("Change energy", () => openEnergy(this.store, () => this.render())),
        btn(this.store.data.calendarStale ? "Refresh calendar" : "Make calendar stale", () => { this.store.data.calendarStale = !this.store.data.calendarStale; this.store.save(); toast(this.store.data.calendarStale ? "Calendar marked stale — auto-reschedule paused." : "Calendar refreshed."); this.render(); }),
      ]),
    ]);
  }
  ...
  private footer(): HTMLElement {
    return el("section", { class: "foot-controls" }, [
      ...
      el("button", { class: "btn btn-ghost sm danger", type: "button", onClick: () => { this.store.reset(); this.timeline = buildTimeline(); this.suggestions = buildSuggestions(); this.render(); toast("Today demo reset."); } }, ["Reset Today"]),
    ]);
  }

  // ---- actions ----
  private applySuggestion(s: ScheduleSuggestion) {
  ...
  private applySafe() {
    ...
    if (steps.length === 0) { toast("No safe changes to apply."); return; }
    openStepPreview({ title: "Apply safe optimisations", steps, store: this.store, trigger: "optimisation (safe)", onDone: () => {
      for (const s of safe) { s.status = "applied"; if (!this.store.data.appliedSuggestions.includes(s.id)) this.store.data.appliedSuggestions.push(s.id); }
      this.store.save(); this.render();
    } });
  }

  private rejectSuggestion(s: ScheduleSuggestion) {
    s.status = "rejected";
    ...
    this.store.save(); this.render(); toast("Removed from your plan.");
  }
  ...
  private restoreSchedule() {
    ...
    this.store.save(); this.render(); toast("Original schedule restored.");
  }

  private blockAction(action: string, b: TimelineBlock) {
    if (action === "Brief me") { openBrief(this.store); return; }
    if (action === "Resolve conflict") { openRecovery(buildRecoveryPlans(), this.store, () => this.render()); return; }
    if (action === "Protect") {
    ...
    if (action === "Ask Buddy to move" || action === "Mark optional") {
      ...
      if (s) this.applySuggestion(s); return;
    }
    toast(`${action} — ${b.title}`);
  }

  private outcomeFeedback(id: string) {
    const body = el("div", { class: "sheet-body" }, [
      el("p", { style: "margin-bottom:14px;" }, ["How does that change feel?"]),
      ...[["useful", "Useful"], ["poor_timing", "Poor timing"], ["intrusive", "Too intrusive"], ["ask_first", "Ask me first next time"]].map(([k, lbl]) =>
        el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { this.store.addFeedback(id, k); toast("Thanks — that shapes future suggestions, not permissions."); (document.querySelector(".sheet-scrim.open .sheet-close") as HTMLElement)?.click(); } }, [lbl])),
  ...
  private openAudit() {
    ...
    openSheet({ title: "Day changes", body: el("div", { class: "sheet-body" }, [
      items.length === 0 ? el("p", { class: "empty" }, ["No changes yet today."]) :
