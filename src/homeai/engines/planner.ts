/**
 * Daily Operating Plan generator. Builds recommendations from the structured
 * signals in mock.ts (not hand-written cards), so the summary numbers always
 * agree with the underlying data.
 */
import type {
  ActionStep, HomeContext, PlanCategory, PlanComparisonRow, PlanSummary, Recommendation,
} from "../models.js";

let seq = 0;
const sid = () => `step-${++seq}`;

function step(partial: Omit<ActionStep, "id" | "executionStatus" | "undoAvailable"> & { undoAvailable?: boolean }): ActionStep {
  return { id: sid(), executionStatus: "suggested", undoAvailable: partial.undoAvailable ?? partial.reversible, ...partial };
}

export function buildRecommendations(ctx: HomeContext): Recommendation[] {
...
export interface PlanState {
  edits: Record<string, { disabled?: boolean; time?: string; handling?: "buddy" | "remind"; instruction?: string; orderDelta?: number }>;
  statuses: Record<string, string>; // recommendation id -> status
  choice: "recommended" | "alternative";
}

export function buildSummary(ctx: HomeContext, recs: Recommendation[], conflictsOpen: number, planState: PlanState): PlanSummary {
...
export function comparePlans(): PlanComparisonRow[] {
  return [
