...
import { clear, el, toast, openSheet } from "../ui/dom.js";
import { ICONS } from "../ui/icons.js";
import { MEETING, PALETTE, PRIMARY_COMMAND, USER } from "./mock.js";
import { AskStore } from "./store.js";
import { openPlanFlow, openConsent, openMomWorkspace, openCompletion, openMonitoring, openActivity } from "./sheets.js";
const MODES = [
...
export class AskApp {
    ...
    render() {
        clear(this.root);
        this.root.append(this.header(), this.contextBar(), this.conversation(), this.composer());
        this.scrollToEnd();
    }
    scrollToEnd() {
    ...
    contextBar() {
        ...
        return el("section", { class: "ctx-bar", "aria-label": "Active context" }, [
            ...
            el("div", { class: "ctx-apps", "aria-label": "Connected apps" }, c.apps.map((a) => el("span", { class: "app-chip", title: `${a} · simulated integration` }, [a[0]]))),
    ...
    openScopePicker() {
        ...
        const body = el("div", { class: "sheet-body" }, [
            ...
            ...scopes.map(([label, type]) => el("button", { class: `btn btn-ghost full${this.store.data.context.scopeLabel === label ? " ctx-active" : ""}`, type: "button", onClick: () => {
                    ...
                    this.store.data.context.scopeLabel = label;
                    this.store.data.context.scopeType = type;
    ...
    openModePicker() {
        const body = el("div", { class: "sheet-body" }, [
            ...MODES.map((m) => el("button", { class: `mode-row${this.store.data.context.mode === m.id ? " active" : ""}`, type: "button", onClick: () => {
                    this.store.data.context.mode = m.id;
                    this.store.save();
                    document.querySelector(".sheet-scrim.open .sheet-close")?.click();
                    this.render();
                    toast(`Mode: ${m.label}`);
    ...
    emptyState() {
        ...
        return el("div", { class: "ask-empty" }, [
            ...
            el("div", { class: "prompt-grid" }, prompts.map(([label, cmd]) => el("button", { class: "prompt-card", type: "button", onClick: () => this.submit(cmd) }, [
    ...
    messageEl(m) {
        if (m.kind === "user")
            return el("div", { class: "bubble user" }, [m.text ?? ""]);
        if (m.kind === "text")
            return el("div", { class: "bubble buddy" }, [el("span", { class: "buddy-mark", html: ICONS.spark }), el("span", {}, [m.text ?? ""])]);
        if (m.kind === "plan")
            return this.planMessage(m.text ?? "");
    ...
    composer() {
        const input = el("textarea", { class: "composer-input", rows: "1", placeholder: "Ask Buddy, or describe what you want done…", "aria-label": "Ask Buddy",
            onInput: (e) => { this.composerValue = e.target.value; const t = e.target; t.style.height = "auto"; t.style.height = Math.min(96, t.scrollHeight) + "px"; },
            onKeydown: (ev) => { const e = ev; if (e.key === "Enter" && !e.shiftKey) {
                e.preventDefault();
                this.submit(this.composerValue);
            } } });
        input.value = this.composerValue;
        const tool = (label, icon, onClick) => el("button", { class: "composer-tool", type: "button", "aria-label": label, title: label, html: icon, onClick });
