...
export function simulateDay(input) {
    ...
    if (input.appliedSuggestions.has("s-focus"))
        focusMins += 0; // already in timeline; keeps it protected
    if (input.appliedSuggestions.has("s-buffer"))
        bufferMins += 10;
    ...
    for (let i = 1; i < t.length; i++)
        if (t[i].type !== t[i - 1].type)
            switches++;
    if (input.appliedSuggestions.has("s-buffer"))
        switches = Math.max(0, switches - 2);
 ...
 */
export function estimateCompletion(scenario, input, healthConsent, energy) {
    ...
    if (m.contextSwitches >= 8)
        negative.push(`${m.contextSwitches} context switches — heavy task-switching`);
    else
        positive.push(`Only ${m.contextSwitches} context switches`);
...
export function energyRecommendations(energy) {
    switch (energy) {
        case "energised":
            return [
