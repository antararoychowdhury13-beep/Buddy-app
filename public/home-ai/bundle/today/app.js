...
import { estimateCompletion, simulateDay } from "./engines.js";
import { TodayStore } from "./store.js";
import { openBrief, openCompanion, openCompare, openCompletion, openDecomposition, openEnergy, openRecovery, openStepPreview, openSuggestionWhy, } from "./sheets.js";
const TYPE_META = {
...
export class TodayApp {
    ...
    constructor(root) {
        ...
        for (const s of this.suggestions) {
            if (this.store.data.appliedSuggestions.includes(s.id))
                s.status = "applied";
            if (this.store.data.rejectedSuggestions.includes(s.id))
                s.status = "rejected";
        }
        this.render();
    }
    ...
    staleBanner() {
        if (!this.store.data.calendarStale)
            return null;
    ...
    applySuggestion(s) {
        openStepPreview({ title: s.title, steps: s.steps, store: this.store, trigger: "optimisation", onDone: () => {
                s.status = "applied";
                if (!this.store.data.appliedSuggestions.includes(s.id))
                    this.store.data.appliedSuggestions.push(s.id);
                this.store.save();
                this.render();
    ...
    rejectSuggestion(s) {
        s.status = "rejected";
        if (!this.store.data.rejectedSuggestions.includes(s.id))
            this.store.data.rejectedSuggestions.push(s.id);
    ...
    outcomeFeedback(id) {
        const body = el("div", { class: "sheet-body" }, [
            ...
            ...[["useful", "Useful"], ["poor_timing", "Poor timing"], ["intrusive", "Too intrusive"], ["ask_first", "Ask me first next time"]].map(([k, lbl]) => el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { this.store.addFeedback(id, k); toast("Thanks — that shapes future suggestions, not permissions."); document.querySelector(".sheet-scrim.open .sheet-close")?.click(); } }, [lbl])),
