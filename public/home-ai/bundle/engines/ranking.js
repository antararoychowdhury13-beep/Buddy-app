...
function scoreRecommendation(r, prefs) {
    ...
    let s = WEIGHTS.urgency * r.urgency +
...
function reasonFor(r) {
    if (r.peopleAffected >= 2)
        return `Ranked high because it blocks ${r.peopleAffected} teammates and is due before your 11 AM meeting.`;
    if (r.emotionalImportance >= 0.9)
        return `Ranked high because it's a protected family commitment with a hard start time.`;
    if (r.financialImpact > 0)
        return `Ranked here because it's money due soon with a late fee if missed, but it's a quick action.`;
    if (r.category === "protect")
        return `Ranked here because it safeguards your only unbroken focus window today.`;
    if (r.buddyCanHandle)
        return `Lower priority for you because Buddy can handle most of it with your review.`;
...
export function rankPriorities(input) {
    const items = input.recommendations
        ...
        return {
            id: `pri-${r.id}`, refId: r.id, kind: "recommendation",
