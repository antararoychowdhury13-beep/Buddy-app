export function selectMode(ctx) {
    if (ctx.travel)
        return { mode: "travel", reason: "A trip is active on your calendar." };
    if (ctx.stressLoad >= 0.7)
        return { mode: "high_stress", reason: "Poor sleep plus a dense meeting day — showing less, prioritising more." };
    if (ctx.dayOfWeek === 0 || ctx.dayOfWeek === 6)
        return { mode: "weekend", reason: "It's the weekend — leading with life, not work." };
    if (ctx.hour < 11)
        return { mode: "morning", reason: "It's the start of your workday." };
    if (ctx.hour >= 18)
        return { mode: "evening", reason: "The workday is winding down." };
...
export function currentContext(signals, overrideStress) {
    const now = new Date();
    return {
        hour: now.getHours(),
...
export const ALL_MODES = ["morning", "workday", "evening", "weekend", "travel", "high_stress"];
