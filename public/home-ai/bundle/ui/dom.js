/** Tiny DOM helpers — no framework, semantic elements, accessible by default. */
...
export function announce(msg) {
    ...
    live.textContent = "";
    // rAF so screen readers register the change
    requestAnimationFrame(() => { live.textContent = msg; });
}
/** Focus-trapping accessible sheet/dialog. Escape closes; focus returns to opener. */
export function openSheet(opts) {
    ...
    const focusables = () => Array.from(sheet.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter((e) => !e.hasAttribute("disabled") && e.offsetParent !== null);
    function onKey(e) {
        if (e.key === "Escape") {
            e.preventDefault();
            close();
    ...
    function close() {
        ...
        setTimeout(() => { scrim.remove(); opener?.focus?.(); }, 220);
    }
    return { close };
}
