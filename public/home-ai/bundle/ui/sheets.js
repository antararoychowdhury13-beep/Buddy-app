import { needsConfirmation, riskLabel, hasPermission } from "../engines/policy.js";
import { comparePlans, COMPARISON_CAVEAT } from "../engines/planner.js";
import { simulateStep } from "../engines/execution.js";
import { announce, el, openSheet, toast } from "./dom.js";
...
let DATA_SOURCES = [];
export function setSheetDataSources(ds) { DATA_SOURCES = ds; }
...
export function openSourceSheet(sourceId, store) {
    const src = DATA_SOURCES.find((s) => s.id === sourceId);
    if (!src)
        return;
...
export function openWhySheet(rec, store) {
...
export function openPlanPreview(recs, store, onDone) {
            ...
            return el("div", { class: `preview-row risk-${step.riskLevel}` }, [
                el("label", { class: "preview-check" }, [
                    el("input", {
                        ...
                        onChange: (ev) => { const c = ev.target; if (c.checked)
                    ...
                    need && isSel ? el("label", { class: `confirm-line${permBlocked ? " blocked" : ""}` }, [
                        el("input", { type: "checkbox", checked: isConf, disabled: permBlocked, "aria-label": `Explicit confirmation for ${step.description}`,
                            onChange: (ev) => { const c = ev.target; if (c.checked)
                                ...
                                confirmed.delete(step.id); updateRun(); } }),
                        permBlocked
...
export function openUndoLearnSheet(rec, store) {
    const body = el("div", { class: "sheet-body" }, [
...
export function openComparePlans(store, onChoose) {
    ...
    function renderSeg() {
        seg.replaceChildren(el("button", { class: `seg-btn${choice === "recommended" ? " active" : ""}`, role: "radio", "aria-checked": choice === "recommended", type: "button", onClick: () => { choice = "recommended"; renderSeg(); } }, ["Recommended"]), el("button", { class: `seg-btn${choice === "alternative" ? " active" : ""}`, role: "radio", "aria-checked": choice === "alternative", type: "button", onClick: () => { choice = "alternative"; renderSeg(); } }, ["Alternative"]));
    }
    renderSeg();
    const body = el("div", { class: "sheet-body" }, [
        seg, table,
...
export function openConflictSheet(c, store, onResolved) {
    ...
    function render() {
        ...
        wrap.replaceChildren(el("div", { class: "info-block" }, [
            el("div", { class: "conf-row" }, [
        ...
        ]), el("div", { class: "res-grid", role: "radiogroup", "aria-label": "Resolution options" }, [optionCard(c.recommended), optionCard(c.alternative)]), el("div", { class: "sheet-actions col" }, [
            ...
            el("button", { class: "btn btn-ghost full", type: "button", onClick: () => { c.status = "deferred"; store.audit("Deferred conflict", c.title, "deferred", true); onResolved(); document.querySelector(".sheet-scrim.open .sheet-close")?.click(); toast("Decide later — Buddy will keep an eye on it."); } }, ["Decide later"]),
        ]), el("p", { class: "fineprint" }, ["Nothing changes until you confirm."]));
    }
        ...
        if (needsConfirm) {
            ...
            const confirmBody = el("div", { class: "sheet-body" }, [
                ...
                permBlocked
                    ...
                    : el("button", { class: "btn btn-primary full", type: "button", onClick: () => { document.querySelectorAll(".sheet-scrim.open .sheet-close")[1]?.click(); proceed(); } }, [`Confirm this ${chosen.riskLevel}-risk action`]),
...
export function openPriorityActions(item, store, onChange) {
    const actions = [
    ...
    if (item.buddyCanHandle)
        actions.splice(2, 0, { label: "Delegate to Buddy", run: () => { store.data.recStatuses[item.refId] = "accepted"; store.save(); } });
    const body = el("div", { class: "sheet-body" }, [
