...
import { BASKET, CAR, CARE_STEPS, CORRELATIONS, FINANCIAL_ACTION, FORECAST_7D, INFO_LEGEND, MICRO, OCCASION, PAL, PERMISSION_CATEGORIES, PERMISSION_LABELS, SAFE_TO_SPEND, SCENARIO, SHARED_WORKFLOWS, STORES, WELLBEING, } from "./mock.js";
function closeTop() { const s = document.querySelectorAll(".sheet-scrim"); s[s.length - 1]?.querySelector(".sheet-close")?.click(); }
function stateMeta(state) { return INFO_LEGEND.find((l) => l.state === state) ?? INFO_LEGEND[0]; }
function infoChip(state) { const m = stateMeta(state); return el("span", { class: "state-chip", style: `color:${m.color}; background:${m.color}1a;` }, [el("span", { class: "dot", style: `background:${m.color};` }), m.label]); }
function visLabel(v) { return { only_you: "Only you", you_and_caregiver: "You & caregiver", household: "Household", custom: "Custom" }[v] ?? v; }
function confMeta(c) { return { high: ["High confidence", PAL.green], medium: ["Medium confidence", PAL.amber], low: ["Low confidence", PAL.red] }[c] ?? ["", PAL.slate]; }
function confBadge(c) { const [label, color] = confMeta(c); return el("span", { class: "conf-chip", style: `color:${color}; background:${color}1a;` }, [label]); }
function block(label, node) { return el("div", { class: "info-block" }, [el("div", { class: "info-label" }, [label]), node]); }
function kv(k, v) { return el("div", { class: "preview-field" }, [el("span", { class: "pf-k" }, [k]), el("span", { class: "pf-v" }, [v])]); }
// ============ MEMBER DETAIL + INFO STATES ============
export function openMember(m, store, rerender) {
    const wrap = el("div", {});
    function render() {
        const items = m.info.filter((i) => !store.data.deletedInfo.includes(i.id));
        wrap.replaceChildren(el("div", { class: "member-hero" }, [
        ...
        ]), m.manages.length ? block("Normally manages", el("div", { class: "chip-row" }, m.manages.map((x) => el("span", { class: "soft-chip" }, [x])))) : el("p", { class: "muted sm" }, ["No responsibilities assigned."]), el("div", { class: "info-label", style: "margin-top:8px;" }, ["Information Buddy holds"]), ...items.map((i) => infoRow(i, store, render, rerender)), el("p", { class: "fineprint" }, ["Every item shows its source and who can see it. You can confirm, correct, restrict or delete anything."]));
    }
    render();
...
function openVisibility(i, store, render) {
    ...
    openSheet({ title: "Who can see this", body: el("div", { class: "sheet-body" }, [
            ...
            ...opts.map(([v, label]) => el("button", { class: `btn btn-ghost full${i.visibility === v ? " ctx-active" : ""}`, type: "button", onClick: () => { i.visibility = v; store.audit("Changed visibility", `${i.label} → ${label}`, "family"); store.save(); closeTop(); render(); toast(`Visibility: ${label}`); } }, [label])),
            el("p", { class: "fineprint" }, ["Changing visibility never shares past data retroactively."]),
        ]) });
}
// ============ NUDGE (trust envelope) ============
export function openNudge(n, store, rerender) {
...
// ============ CARE JOURNEY ============
export function openCare(store, rerender) {
    const wrap = el("div", {});
...
// ============ WELLBEING ============
export function openWellbeing(store, rerender) {
    const wrap = el("div", {});
...
// ============ HOUSEHOLD BASKET ============
export function openBasket(store, rerender) {
    const wrap = el("div", {});
    ...
    function render() {
        ...
        wrap.replaceChildren(el("div", { class: "consent-lead" }, [el("span", { class: "ic", html: ICONS.shield }), el("p", {}, ["Buddy prepared this basket — nothing is bought until you confirm."])]), ...items.map((b) => el("div", { class: `basket-item${b.included ? "" : " excluded"}` }, [
            el("label", { class: "basket-check" }, [el("input", { type: "checkbox", checked: b.included, "aria-label": `Include ${b.name}`, onChange: (e) => { b.included = e.target.checked; render(); } })]),
...
// ============ ASSET / CAR ============
export function openAsset(store, rerender) {
    const a = CAR;
...
// ============ OCCASION ============
export function openOccasion(store, rerender) {
    const o = OCCASION;
    const wrap = el("div", {});
    function render() {
        const italianConfirmed = store.data.confirmedInfo.includes("i-p2");
        wrap.replaceChildren(el("div", { class: "occ-hero" }, [el("div", { class: "occ-hero-title" }, [o.title]), el("div", { class: "occ-hero-sub" }, [`${o.date} · ${o.people.join(" & ")} · budget ₹${o.budget.toLocaleString("en-IN")}`])]), block("Gift & plan ideas", el("div", { class: "idea-list" }, o.giftIdeas.map((g) => el("div", { class: "idea-row" }, [
            el("div", { style: "flex:1;" }, [
...
// ============ MONEY ============
export function openMoney(store, rerender) {
    let tab = "safe";
    const wrap = el("div", {});
    function render() {
        const seg = el("div", { class: "seg", role: "radiogroup", "aria-label": "Money view" }, [["safe", "Safe to spend"], ["forecast", "Forecast"], ["scenario", "Scenarios"]].map(([k, label]) => el("button", { class: `seg-btn${tab === k ? " active" : ""}`, role: "radio", "aria-checked": tab === k, type: "button", onClick: () => { tab = k; render(); } }, [label])));
        const body = [seg];
        if (tab === "safe")
            body.push(safeToSpendView());
        else if (tab === "forecast")
            body.push(forecastView());
        else
            body.push(scenarioView(store, rerender));
...
export function openFinancial(fa, store, rerender) {
...
// ============ SHARED WORKFLOW ============
export function openSharedWorkflow(store, rerender) {
...
// ============ PERMISSIONS ============
export function openPermissions(store, rerender) {
    const wrap = el("div", {});
    const levels = Object.keys(PERMISSION_LABELS);
    function render() {
        wrap.replaceChildren(el("p", { class: "muted", style: "margin-bottom:12px;" }, ["What Buddy may do per category. Set separately for each — nothing escalates automatically."]), ...PERMISSION_CATEGORIES.map((c) => {
            const cur = store.data.permissions[c.id] ?? c.level;
            return el("div", { class: "perm-row" }, [
                el("div", { class: "perm-cat" }, [c.label]),
                el("select", { class: "perm-select", "aria-label": `Permission for ${c.label}`, onChange: (e) => { store.data.permissions[c.id] = e.target.value; store.audit("Changed permission", `${c.label} → ${PERMISSION_LABELS[e.target.value]}`, c.id); store.save(); render(); } }, levels.map((l) => el("option", { value: l, selected: l === cur }, [PERMISSION_LABELS[l]]))),
            ]);
