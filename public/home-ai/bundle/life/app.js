...
import { HOUSEHOLD, HOUSEHOLD_SOLO, INFO_LEGEND, NEEDS_ATTENTION, PAL, SAFE_TO_SPEND } from "./mock.js";
import { LifeStore } from "./store.js";
import { openMember, openCare, openWellbeing, openBasket, openAsset, openOccasion, openMoney, openSharedWorkflow, openNudge, openPermissions, openActivity, } from "./sheets.js";
export class LifeApp {
    store = new LifeStore();
    root;
    constructor(root) { this.root = root; this.render(); }
    household() { return this.store.data.household === "hh-solo" ? HOUSEHOLD_SOLO : HOUSEHOLD; }
    privacy() { return this.store.data.privacyMode; }
    render() {
        clear(this.root);
        const sections = [
            this.header(),
    ...
    familyCare() {
        ...
        return el("section", { class: "life-sec", "aria-label": "Family and care" }, [
            ...
            el("article", { class: "care-card", onClick: () => openCare(this.store, () => this.render()) }, [
                ...
                el("button", { class: "btn btn-primary full", type: "button", style: `background:${PAL.green}; box-shadow:none;`, onClick: (e) => { e.stopPropagation(); openCare(this.store, () => this.render()); } }, ["Open care journey"]),
            ]),
        ]);
    }
    wellbeing() {
